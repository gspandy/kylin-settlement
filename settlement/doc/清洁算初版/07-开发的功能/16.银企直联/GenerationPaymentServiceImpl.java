...
//				return CodeEnum.FAILURE.getCode();
				...
				logger.info("代收付推送实时代收/付的结果   开始  ==订单号="+orderDetail.getOrderNo()+" 机构号=="+orderDetail.getRootInstCd());
				CommonResponse response = recAndPaySysPushRealTimeRec(orderDetail);
				logger.info("代收付推送实时代收/付的结果   结束  ==订单号="+orderDetail.getOrderNo()+" 机构号=="+orderDetail.getRootInstCd()+" Code="+response.getCode()+"(1为成功,成功指代收付返回成功，并且处理成功，其他失败), errMsg="+response.getMsg());
			}
		}else{
			try {
				Thread.sleep(2000);
...
//			String oneKey = CommUtil.getGenerateId();
	...
	private CommonResponse recAndPaySysPushRealTimeRec(OrderDetail orderDetail){
		...
		logger.info("代收付推送实时代收/付的结果==orderNo="+orderNo+" instCode=="+instCode+" statusId="+orderDetail.getStatusId());
		...
		logger.info("代收付推送实时代收/付的结果,根据订单号=="+orderNo+",机构号 =="+instCode+" 查出的数据个数="+transOrderInfoList.size());
		...
		if (transOrderInfo.getStatus()!=TransCodeConst.TRANS_STATUS_PROCESSING) {
			logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+",Status="+transOrderInfo.getStatus()+" 该数据不是处理中状态=");
			res.setCode(CodeEnum.FAILURE.getCode());
			res.setMsg("数据不是处理中状态");
			return res;
		}
		if(orderDetail.getStatusId()==15){//代收付系统返回成功
			if(!settlementServiceThr.isDayEndOk()){
				logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+"   日终没有正常结束！");
			...
			if (TransCodeConst.PAYMENT_REAL_TIME_COLLECTION.equals(transOrderInfo.getFuncCode())) {
		           ErrorResponse response=paymentAccountService.collectionRealTime(transOrderInfo);
		            logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+" 生成流水及订单信息 response.isIs_success()="+response.isIs_success()+" msg ="+response.getMsg());
		            //如果失败，修改订单信息状态
...
//		              transOrderInfo.setStatus(TransCodeConst.TRANS_STATUS_INVALIDATION);
            ...
            }else if(TransCodeConst.PAYMENT_REALTIME.equals(transOrderInfo.getFuncCode())){
                TransOrderInfo transOrderInfoUp =  new TransOrderInfo();
                transOrderInfoUp.setRequestId(transOrderInfo.getRequestId());
                transOrderInfoUp.setTradeFlowNo("0");
                transOrderInfoUp.setStatus(TransCodeConst.TRANS_STATUS_PAY_SUCCEED);
                transOrderInfoDao.updateByPrimaryKeySelective(transOrderInfoUp);
            }
		}else{ //代收付系统返回失败
			logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+" 代收付返回失败 orderDetail.getRetCode()="+orderDetail.getRetCode()+" orderDetail.getErrMsg() ="+orderDetail.getErrMsg());
			transOrderInfo.setStatus(TransCodeConst.TRANS_STATUS_PAY_FAILED);
    	...
    	try {
			...
			e.printStackTrace();
			logger.info("代收付推送实时代收付的结果,订单号=="+transOrderInfo.getOrderNo()+",机构号 =="+transOrderInfo.getMerchantCode()+" 调用订单系统异常");
			res.setCode(CodeEnum.FAILURE.getCode());
			res.setMsg("调用订单系统异常！");
			return res;
		}
    	logger.info("代收付推送实时代收付的结果,订单号=="+transOrderInfo.getOrderNo()+",机构号 =="+transOrderInfo.getMerchantCode()+" 订单系统返回的数据rtnMapUp="+rtnMapUp);
     	if(!"true".equals(rtnMapUp.get("issuccess"))){
			...
			if("1".equals(rtnOrderPayList.get(0).getStatusId())){
				logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+"  在订单系统找不到数据");
				res.setMsg("在订单系统找不到数据！");
			}else{
				logger.info("代收付推送实时代收付的结果,订单号=="+orderNo+",机构号 =="+instCode+"  订单系统数据状态已更新");
...
//		}
	...
	private TransOrderInfoResponse transferToRepay(List<GenerationPayment> genList){
		...
		for (TransOrderInfo transOrderInfo : transOrderInfoList) {
			...
			transOrderInfo.setFuncCode(TransCodeConst.FUNCTION_TRANSFER);
			Date date = DateUtils.getSysDate(Constants.DATE_FORMAT_YYYYMMDDHHMMSS);
	 ...
	 * Discription:
	 * @param transOrderInfo
	 * @param params   参数顺序依次:业务编码,业务类型,订单类型
	 * @return OrderDetail
	 * @author Achilles
	 * @since 2017年3月22日
	 */
	private OrderDetail getOrderDetailByTransOrderInfo(TransOrderInfo transOrderInfo,String[] params){
		OrderDetail orderDetail = new OrderDetail(); //CommUtil
		String batch = CommUtil.getGenerateNum(4);
		orderDetail.setRequestNo(batch);//批号
        orderDetail.setBussinessCode(params[0]);
        orderDetail.setBussinessType(Integer.parseInt(params[1]));//业务类型ID:0实时;1非实时
        ...
        orderDetail.setOrderType(Integer.parseInt(params[2]));
		...
		if (CrpsConstants.ORDER_TYPE_PAYMENT.equals(params[2])) {
		    orderDetail.setOrgCode(transOrderInfo.getPayChannelId()); 
        }
		return orderDetail; 
	}
	...
	public String submitToCollRealTime(TransOrderInfo transOrderInfo){
        OrderDetail orderDetail = getOrderDetailByTransOrderInfo(transOrderInfo,new String[]{SettleConstants.COLLECTION_CODE,
                String.valueOf(CrpsConstants.BUSSINESS_TYPE_REAL_TIME),CrpsConstants.ORDER_TYPE_COLLECTION});
		if (orderDetail==null) {
...
//		}
	...
	public void submitToRecAndPaySys(int type,String rootInstCd,String accDate,Date accountDate){
		...
		if(newType != -1){
            ...
            query.setRemark(TransCodeConst.SUMMARY_ALL);
		...
		List<GenerationPayment> generationPaymentListSel=generationPaymentManager.queryListByOrderType(query);
		logger.info("---业务类型="+type+"(6:代收，7：代付),机构号="+rootInstCd+",查出的要发送到代收付系统的个数="+generationPaymentListSel.size());
		if(generationPaymentListSel.size()==0){
			return;
		}
		List<List<GenerationPayment>> bigList = new ArrayList<List<GenerationPayment>>();
        List<GenerationPayment> newSaveList = new ArrayList<GenerationPayment>();
        String oldOrgCode = "noResult";
        for(int i = 0 ; i < generationPaymentListSel.size() ; i ++){
            GenerationPayment gp = generationPaymentListSel.get(i);
            String requestNo = gp.getRequestNo();
            if(CommUtil.isEmp(requestNo)){
                newSaveList.add(gp);
                if((i + 1) == generationPaymentListSel.size()){
                    bigList.add(newSaveList);
                }
            }else if(oldOrgCode.equals(requestNo)){
                newSaveList.add(gp);
                if((i + 1) == generationPaymentListSel.size()){
                    bigList.add(newSaveList);
                }
            	...
            	bigList.add(newSaveList);
                oldOrgCode = requestNo;
                newSaveList = new ArrayList<GenerationPayment>();
                newSaveList.add(gp);
                if((i + 1) == generationPaymentListSel.size()){
                    bigList.add(newSaveList);
                }
        ...
        //所有发送到代收付成功的代收付交易
        List<GenerationPayment> genPaySuccessAllList = new ArrayList<GenerationPayment>();
        for(List<GenerationPayment> generationPaymentList : bigList){
            if(generationPaymentList == null || generationPaymentList.isEmpty()){
                continue;
            }
          //先生成一个批次号，少于limitCount的时候直接使用;或者大于limitCount的时候第一批limitCount使用
            String batch = newType == -1 ? getBatchNo(type, rootInstCd, accDate, null) : "YFYZ" + System.currentTimeMillis();
            ...
            for (int i = 1; i <= generationPaymentList.size(); i++) {
                ...
                //新增   在发送给代收付/多渠道时 将该值传入他们接口中的orgCode中
                String orgCode = generationPayment.getRequestNo();
                if(!CommUtil.isEmp(orgCode)){
                ...
                if(!lastGreaterLimit && generationPaymentList.size()-i<limitCount){
                    batch = newType == -1 ? getBatchNo(type, rootInstCd, accDate, batch) : "YFYZ" + System.currentTimeMillis();
                    generationPayment.setProcessResult(batch);// 批次号
                    genPaySuccessList.add(generationPayment);
                    orderDetail.setRequestNo(batch);
                    orderDeBatchList.add(orderDetail);
...
//			orderDetail.setPayBankName(generationPayment.getp);
	...
	/**
	 * 根据代收付系统查出的实时代收付结果修改账户系统
	 * @param orderNoArray
	 */
	public CommonResponse updateRealTimeTransInfoByRecAndPaySys(String[] orderNoArray){
		logger.info("根据代收付系统查出的实时代收付结果修改账户系统    开始  ==============orderNoArray==="+orderNoArray);
		CommonResponse res = new CommonResponse();
		if(!settlementServiceThr.isDayEndOk()){
			logger.info("根据代收付系统查出的实时代收付结果修改账户系统  日终没有正常结束！");
			res.setCode(CodeEnum.ERR_SYS_DAYEND_NOT_OK.getCode());
			res.setMsg(CodeEnum.ERR_SYS_DAYEND_NOT_OK.getMessage());
			return res;
		}
		...
		try {
			...
			e.printStackTrace();
			logger.error("根据代收付系统查出的实时代收付结果修改账户系统  调用代收付系统异常 =====");
			res.setCode(CodeEnum.ERR_CRPS_CALL_ERROR.getCode());
			res.setMsg(CodeEnum.ERR_CRPS_CALL_ERROR.getMessage());
			return res;
		}
		logger.info("根据代收付系统查出的实时代收付结果修改账户系统   代收付返回的 ==OrderDetails==="+orderDetails);
		List<OrderDetail> orderDeList = null;
		if (orderDetails == null || orderDetails.getOrderDetails()==null ||orderDetails.getOrderDetails().size()==0) {
			logger.info("根据代收付系统查出的实时代收付结果修改账户系统   代收付返回的 ==OrderDetails为空");
			res.setCode(CodeEnum.ERR_CRPS_NO_RESULT.getCode());
			res.setMsg(CodeEnum.ERR_CRPS_NO_RESULT.getMessage());
			orderDeList  = new ArrayList<OrderDetail>();
		}else{
		...
		logger.info("根据代收付系统查出的实时代收付结果修改账户系统   代收付返回的 ==OrderDetails的个数="+orderDeList.size());
		boolean exist = false;
		...
		for (OrderDetail orderDetail : orderDeList) {
			...
			List<TransOrderInfo> transOrderInfoList =  transOrderInfoManager.queryList(query);
			logger.info("根据代收付系统查出的实时代收付结果修改账户系统,订单号=="+orderDetail.getOrderNo()+",机构号 =="+orderDetail.getRootInstCd()+",status="+orderDetail.getStatusId()+"(代收付返回的，非查询条件) 查出的数据个数="+transOrderInfoList.size());
			if (transOrderInfoList.size()==0) {
				res.setCode(CodeEnum.ERR_TRADE_RECORD_NO_RESULT.getCode());
		...
		logger.info("根据代收付系统查出的实时代收付结果修改账户系统  代收付返回的数据=paramMapList=="+paramMapList);
		// 代收付返回成功或失败的订单通知订单系统；
		Map<String,Object> rtnMapUp = null;
    	try {
    		rtnMapUp = updateOrderSysStatus(transOrInfoList);
			...
			e.printStackTrace();
			logger.info("根据代收付系统查出的实时代收付结果修改账户系统   机构号 =="+transOrInfoList.get(0).getMerchantCode()+" 调用订单系统异常");
			res.setCode(CodeEnum.ERR_ORDER_CALL_ERROR.getCode());
			res.setMsg(CodeEnum.ERR_ORDER_CALL_ERROR.getMessage());
			return res;
		}
    	logger.info("根据代收付系统查出的实时代收付结果修改账户系统   机构号 =="+transOrInfoList.get(0).getMerchantCode()+" 订单系统返回的数据rtnMapUp="+rtnMapUp);
     	if(!"true".equals(rtnMapUp.get("issuccess"))){
			List<OrderPayment> rtnOrderPayList =  (List<OrderPayment>)rtnMapUp.get("paymentList");
			res.setCode(CodeEnum.FAILURE.getCode());
			for (OrderPayment orderPayment : rtnOrderPayList) {
				if("1".equals(orderPayment.getStatusId())){
					logger.info("根据代收付系统查出的实时代收付结果修改账户系统,订单号=="+orderPayment.getPaymentId()+"  在订单系统找不到数据");
					res.setMsg("订单号=="+orderPayment.getPaymentId()+" 在订单系统找不到数据！");
				}else{
					logger.info("根据代收付系统查出的实时代收付结果修改账户系统,订单号=="+orderPayment.getPaymentId()+"  订单系统数据状态已更新");
...
//        }
    ...
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, readOnly = false)
    @Override
    public  CommonResponse submitToGateRouTransfer(String accountDate,Integer sendType,String protocol,String rootInstCd){
        CommonResponse res = new CommonResponse();
        res.setCode(CodeEnum.FAILURE.getCode());
        if (CommUtil.isEmp(protocol)) {
            res.setMsg("协议为空!");
        ...
        paymentDto.setSysNo(GatewayConstants.SYS_NO_ACCOUNT_003);
        paymentDto.setTransCode("16023");
        paymentDto.setOrgNo(protocol);
        paymentDto.setBusiCode("16011");// 业务编码
        paymentDto.setChannelNo("160725");
        paymentDto.setSignType(1);// 签名类型 固定值1 即MD5
        ParameterInfo parameterInfo = parameterInfoService.getParameterInfoByParaCode(GatewayConstants.GATEROUTER_MD5_KEY);
        String md5Key = parameterInfo.getParameterValue();
        paymentDto.setSignMsg(paymentDto.sign(md5Key));
        paymentDto.setCurrency(BaseConstants.CURRENCY_CNY);
        paymentDto.setSettleFlag(1);//普通
        List<ParameterInfo> parameterInfoList = parameterInfoService.getByParamTypeAndProductId(rootInstCd,"MINSHENG_VER_ACCOUNT");
        ...
        for (GenerationPayment generationPayment : generationPaymentList) {
            paymentDto.setTransNo(generationPayment.getOrderNo());
            paymentDto.setPayerAccountNo(parameterInfoList.get(0).getParameterCode());
            paymentDto.setPayerAccountName(parameterInfoList.get(0).getParameterValue());
            paymentDto.setPayAmount(generationPayment.getAmount());
            paymentDto.setReceiverAccountNo(generationPayment.getAccountNo());
            paymentDto.setReceiverAccountName(generationPayment.getAccountName());
            paymentDto.setReceiverAccountType(GatewayConstants.ACCOUNT_TYPE_PRI);
            ...
            paymentDto.setReceiverBankName(generationPayment.getOpenBankName());
            paymentDto.setBankFlag(2);//行外
            if (generationPayment.getAmount()<=5000000) {//5w以上填大额,5w以下填小额
                paymentDto.setRemitWay(3);
            }else{
                paymentDto.setRemitWay(4);
            }
            if ("305".equals(generationPayment.getBankCode())) {
                paymentDto.setBankFlag(1);
            }
            paymentDto.setPurpose("放款");
            paymentDto.setSettleFlag(1);//1普通，2实时，3 特急
            logger.info("调用多渠道转账入参: SysNo=" + paymentDto.getSysNo() + ",TransCode=" + paymentDto.getTransCode() + ",OrgNo="
                    + paymentDto.getOrgNo() + ",BusiCode=" + paymentDto.getBusiCode() + ",SignType="+ paymentDto.getSignType() + ",SignMsg=" 
                    + paymentDto.getSignMsg() + ",Currency="+paymentDto.getCurrency()+",RemitWay="+paymentDto.getRemitWay()
                    +"TransNo="+paymentDto.getTransNo()+",PayAmount="+paymentDto.getPayAmount()+
                    ",Currency="+paymentDto.getCurrency()+",ReceiverAccountNo="+paymentDto.getReceiverAccountNo()
                    +",ReceiverAccountName="+paymentDto.getReceiverAccountName()+",ReceiverAccountType="+paymentDto.getReceiverAccountType()
                    +",ReceiverBankNo="+paymentDto.getReceiverBankNo()+",SettleFlag="+paymentDto.getSettleFlag()+",ReceiverBankName="
                    +paymentDto.getReceiverBankName()+",BankFlag="+paymentDto.getBankFlag()+",Purpose="+paymentDto.getPurpose()
                    +",PayerAccountNo="+paymentDto.getPayerAccountNo()+",PayerAccountName="+paymentDto.getPayerAccountName());
            PaymentRespDto resDto = null;
            try {
                resDto = cmbcAccountService.subAcctPayment(paymentDto);
            } catch (Exception e) {
                logger.error("调用多渠道转账接口异常!", e);
                res.setMsg("系统异常!");
                return res;
            } 
            logger.info("调用多渠道转账返回值:    returnCode=" + resDto.getReturnCode() + ",returnMsg=" + resDto.getReturnMsg()
            +  ",StatusId=" + resDto.getStatusId());
            GenerationPayment genPayment = new GenerationPayment();
            genPayment.setGeneId(generationPayment.getGeneId());
            if (!"100000".equals(resDto.getReturnCode()) ||(resDto.getStatusId()!=10 && resDto.getStatusId()!=12)) {
                genPayment.setSendType(3);
                error++;
            }else{
                genPayment.setSendType(2);
            }
            generationPaymentDao.updateByPrimaryKeySelective(genPayment);
        }
        if (error!=0) {
            res.setMsg("部分失败,或全部失败!");
            return res;
        }
        return new CommonResponse();
    }   
    ...
    @Override
    public String submitToPayRealTime(TransOrderInfo transOrderInfo){
        OrderDetail orderDetail = getOrderDetailByTransOrderInfo(transOrderInfo,new String[]{SettleConstants.WITHHOLD_CODE,
                String.valueOf(CrpsConstants.BUSSINESS_TYPE_REAL_TIME),CrpsConstants.ORDER_TYPE_PAYMENT});
        if (orderDetail==null) {
            logger.info("实时代付  没有查到用户: "+transOrderInfo.getUserId()+" 机构码： "+transOrderInfo.getMerchantCode() +" 结算卡！");
            transOrderInfo.setErrorMsg(CodeEnum.ERR_ACCOUNT_NO_DEBIT_CARD.getMessage());
            return CodeEnum.ERR_ACCOUNT_NO_DEBIT_CARD.getCode();
        }
        logger.info("--实时代付--发送到代收付的参数: RequestNo="+orderDetail.getRequestNo()+",BussinessCode="+orderDetail.getBussinessCode()
                +",BussinessType="+orderDetail.getBussinessType()+",RootInstCd="+orderDetail.getRootInstCd()+",UserId="+orderDetail.getUserId()
                +",CertificateType="+orderDetail.getCertificateType()+",AccountNo="+orderDetail.getAccountNo()+",AccountName="
                +orderDetail.getAccountName()+",accountProp="+orderDetail.getAccountProp()+",AccountType="+orderDetail.getAccountType()
                +",BankCode="+orderDetail.getBankCode()+",BankName="+orderDetail.getBankName()+",PayBankCode="
                +orderDetail.getPayBankCode()+",OrderNo="+orderDetail.getOrderNo()+",OrderType="+orderDetail.getOrderType()
        ...
        try {
            ...
            logger.error("实时代付  订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode() +" 调用代收付系统异常！",e);
            transOrderInfo.setErrorMsg(CodeEnum.ERR_CRPS_CALL_ERROR.getMessage());
            return CodeEnum.ERR_CRPS_CALL_ERROR.getCode(); 
        }
        int statusId = orderDe.getStatusId() == null?0:orderDe.getStatusId();
        String returnCode = orderDe.getRetCode();
        String errorMsg = orderDe.getErrMsg();
        logger.info("实时代付  订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode() +" 调用代收付系统返回statusId="+orderDe.getStatusId()+",returnCode="+returnCode+",errorMsg="+errorMsg);
        //如果returnCode不是100000可能是代收付系统调用多渠道的额异常，具体看errorMsg
        if("4444".equals(returnCode)){
            logger.info("实时代付        外部系统异常      订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode());
            transOrderInfo.setErrorMsg(returnCode+":"+errorMsg);
            return TransCodeConst.TRANS_STATUS_PROCESSING+"";  
        }
        if(!CrpsConstants.RETURN_CODE_SUCCESS.equals(returnCode)){
            logger.info("实时代付  (可能)校验性失败  订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode());
            transOrderInfo.setErrorMsg(statusId+":"+errorMsg);
            return CodeEnum.ERR_PARAM_ABNORMAL.getCode(); 
        }
        //如果是处理中
        if (statusId==CrpsConstants.STATUS_PROCESSING) {
            logger.info("实时代付  处理中  订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode());
            transOrderInfo.setErrorMsg(statusId+":"+errorMsg);
            return TransCodeConst.TRANS_STATUS_PROCESSING+""; 
        }
        //失败
        if (statusId==CrpsConstants.STATUS_FAILURE) {
            logger.info("实时代付  失败  订单号: "+transOrderInfo.getOrderNo()+" 机构码： "+transOrderInfo.getMerchantCode());
            transOrderInfo.setErrorMsg(statusId+":"+errorMsg);
            return CodeEnum.FAILURE.getCode(); 
        }
        return  CodeEnum.SUCCESS.getCode();
