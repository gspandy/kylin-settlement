 ...
 * 汇总悦艺术家交易掉账户调账
 */
function doCollectForAcc(butt) {
	var collectType = $("#collectType2").val();
	var accountDate = $("#collectAccountDate2").val();
	
	if(!(collectType && accountDate)) {
		alert("请输入完整的汇总信息!");
		return;
	}
	...
	var para = "collectType="+collectType+"&accountDateStr="+accountDate;

	$.ajax({
        type: "POST",
        url: $("#doCollectForAcc").val(),
        data: para,
        dataType: "text",
        success: function(data){
			alert("操作完成");
 ...
 * 发送汇总交易记账
 */
function doKernelEntryForAcc(butt) {
	var oldButt = loadingFun(butt);
	var checkedBox = $(".ids_box[name=ids]:checked");
	var idsStr = "";
	checkedBox.each(function() {
