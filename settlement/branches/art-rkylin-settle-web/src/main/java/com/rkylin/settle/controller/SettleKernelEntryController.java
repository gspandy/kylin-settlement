...
public class SettleKernelEntryController extends BaseController {
	 ...
	 * 发送汇总账户一期交易并记账
	 */
	@RequestMapping("/do_collect_for_acc")
	public void doCollectForAcc(String collectType, String accountDateStr) {
		logger.info(">>> >>> >>> 开始 发送汇总账户一期交易并记账 [{collectType:"+collectType+"},{accountDateStr:"+ accountDateStr +"}]");
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
			Date accountDate = sdf.parse(accountDateStr);
			settleWebInterface.doCollectForAccLogic(Integer.parseInt(collectType), accountDate);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("异常 发送汇总账户一期交易并记账", e);
		}
		logger.info("<<< <<< <<< 结束 发送汇总账户一期交易并记账 [{collectType:"+collectType+"},{accountDateStr:"+ accountDateStr +"}]");
	}
	/**
	 * 发送汇总交易记账
	 */
	@RequestMapping("do_kernel_entry_for_acc")
	public void doKernelEntryForAcc(String idsStr) {
		logger.info(">>> >>> >>> 开始  发送汇总交易记账 ID["+ idsStr +"]");
		String[] idsArr = idsStr.split(",");
		...
		for(String idStr : idsArr) {
			id = Long.parseLong(idStr);
			try {
				resultMap = settleWebInterface.doCollectBySettleKernelEntryForAccLogic(id);
				code = resultMap.get("code");
				msg = resultMap.get("msg");
				logger.info(">>> >>> ID:"+ id +", 调用清结算服务 doCollectBySettleKernelEntryForAccLogic 返回值 ["
						+ "code:"
						+ code 
						+ ", msg:" 
						+ msg
		...
		logger.info("<<< <<< <<< 结束  发送汇总交易记账 ID["+ idsStr +"]");
	}
}
