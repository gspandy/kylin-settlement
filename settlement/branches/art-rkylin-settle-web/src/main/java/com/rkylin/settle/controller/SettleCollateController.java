...
//        }
     ...
     * 其他环境对账
     * @param date 选择账期
     * @param otherEnviron 选择环境（uat）
     */
    @RequestMapping("/oeBalanceAccount_ajax")
    public void otherEnvironBalanceAccount_ajax(String date,String otherEnviron,String rule) {
    	logger.info(">>> >>> >>> >>> 开始  其他环境("+otherEnviron+"环境)对账 入参　date:" + date+",otherEnviron:"+otherEnviron);
    	try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			settleWebInterface.otherEnvironCollage(sdf.parse(date), otherEnviron,rule);
			writeJson(null);
		} catch (Exception e) {
			e.printStackTrace();
		}
