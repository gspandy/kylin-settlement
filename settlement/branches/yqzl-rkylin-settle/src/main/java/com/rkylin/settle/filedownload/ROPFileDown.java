...
//	    		invoicedate = parameterInfoManager.queryList(keyList).get(0).getParameterValue();
		...
		} else if (type == 87) {// 杏仁支付回盘文件
	    	logger.info(">>> >>> 编辑杏仁支付回盘文件");
	    	File resFile = new File(ropMap.get("file"));
	    	try {
				int res = dsfLogic.readResFile(resFile);
				if (res == 2) {
					rtnMap.put("errCode", "0002");
					rtnMap.put("errMsg", "文件中存在非终态数据，请检查文件，然后重新上传！");
				}
			} catch (Exception e) {
				logger.error("编辑杏仁支付回盘文件异常！"+e.getMessage());
				e.printStackTrace();
			}
		}
