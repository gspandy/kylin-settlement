...
public class DsfMessageListener extends BaseDao implements MessageListener {
	...
	protected static Logger logger = LoggerFactory
			.getLogger(DsfMessageListener.class);
	...
	public void onMessage(Message msg) {
		logger.info(">>>>> 开始:监听到ActiveMQ队列中代收付的消息！");
		String orderNo = null;
		...
		try {
			...
			returnStatusId = message.getInt("statusId");
			errorMsg = message.getString("errorMsg");
			orgCode = message.getString("orgCode");
			logger.info(">>>监听MQ队列,orderNo=" + orderNo + ",returnStatusId=" + returnStatusId + ",errorMsg=" + errorMsg);
		} catch (JMSException e) {
			e.printStackTrace();
			logger.error(">>> >>> 异常:从message过去信息时异常 orderNo=" + orderNo + ",错误信息:" + e.getMessage(), e);
		}
		/*
		 * 获取协议号和渠道号
		 */
		try {
			orgNumAndChannelHome = this.getOrgNumAndChannelHome(orgCode);
		} catch (Exception e) {
			e.printStackTrace();
			logger.info(">>> >>> 异常:获取协议号和渠道号", e);
			orgNumAndChannelHome = orgCode;
		}
		/*
		 * 根据MQ队列的 orderNo查询结算表
		 */
		logger.info(">>> 根据MQ队列的 orderNo=" + orderNo + " 查询结算表开始");
		SettleTransInvoiceQuery query = null;
		List<SettleTransInvoice> settleTransInvoiceList = null;
		query = new SettleTransInvoiceQuery();
		query.setOrderNo(orderNo);
		try {
			settleTransInvoiceList = settleTransInvoiceManager.queryList(query);
			if (settleTransInvoiceList == null || settleTransInvoiceList.size() != 1) {
				logger.error(">>> 异常:settleTransInvoiceList is Null or settleTransInvoice is Not Unique");
				return;
			}
			invoiceBean = settleTransInvoiceList.get(0);
			if(invoiceBean == null) {
				logger.error(">>> >>> 异常:将代收付的结果更新到结算表 invoiceBean = settleTransInvoiceList[0] is Null");
				return;
			}
		} catch(Exception e) {
			logger.error(">>> >>> 异常:根据MQ队列的 orderNo查询结算表异常", e);
			return;
		}
		/*
		 * 将代收付的结果更新到结算Invoice表
		 */
		try {
			invoiceBean = this.updateInvoiceProcessResult(invoiceBean, returnStatusId, errorMsg, orgNumAndChannelHome);
		} catch (Exception e) {
			logger.error(">>> >>> 异常:将代收付的结果更新到结算表", e);
			return;
		}
		if (SettleConstants.DSF_HAS_WRITE != invoiceBean.getStatusId()) {//监听MQ队列,statusId=6 已经回写过数据,此处去重去重,不再处理
			/*
			 * 将代收付结果更新到汇总表
			 */
			try {
				sendToOrderList = this.updateSummeryStatusId(invoiceBean, returnStatusId, errorMsg);
				invoiceBean.setStatusId(SettleConstants.DSF_HAS_WRITE);
				settleTransInvoiceManager.updateSettleTransInvoice(invoiceBean);
			} catch (Exception e) {
				logger.error(">>> >>> 异常: 将代收付结果更新到汇总表 updateSummeryStatusId:", e);
			}
			/*
			 * 代收付结果 发送账户
			 */
			...
			logger.info("监听MQ队列,statusId=6 已经回写过数据,此处去重去重,不再处理,orderNo=" + invoiceBean.getOrderNo() + ", statusId=" + invoiceBean.getStatusId());
		}
		logger.info("<<<< 结束:监听到ActiveMQ队列中代收付的消息！orderNo=" + orderNo);
	}
	/**
	 * 返回代付结果给账户系统
	 * 
...
//			this.removeSuccessToHis();					// 移入历史
			...
			logger.error("返回代付结果给账户系统发生异常，error=" + e.getMessage(), e);
		}
	}
	...
	public SettleTransInvoice updateInvoiceProcessResult(SettleTransInvoice settleTransInvoice, int returnStatusId, String errorMsg, String orgNumAndChannelHome) {
		logger.info(">>>>>> 开始:将代收付的结果更新到结算表, orderNo=" + settleTransInvoice.getOrderNo() + ", 代收付返回的结果=" + returnStatusId);
		if (SettleConstants.DSF_HAS_RETURN != settleTransInvoice.getStatusId()) {
			...
			logger.info("代收付的结果更新到结算表时发现状态是5已经返回过了,此处不再处理,orderNo="
					+ settleTransInvoice.getOrderNo() + "statusId="
					+ settleTransInvoice.getStatusId());
		}
		logger.info(">>>>>> 结束:将代收付的结果更新到结算表,orderNo=" + settleTransInvoice.getOrderNo() + ",代收付返回的结果=" + returnStatusId);
	 ...
	 */
	public Map<String, List<SettleTransDetail>> updateSummeryStatusId(SettleTransInvoice invoiceBean, int returnStatusId, String errorMsg) {
...
//		List<SettleTransDetail> sendToOrderList = new ArrayList<SettleTransDetail>();
		...
		try {
			...
			if (summaryList != null) {
				logger.info(">>>MQ的监听类中,根据结算表的batchNo从汇总表查询数据,batchNo="
						+ batchNo + ",共查询到：" + summaryList.size() + "条");
			} else {
				logger.info(">>>MQ的监听类中,根据结算表的batchNo从汇总表查询数据,batchNo="
						+ batchNo + ",共查询到0条");
			}
...
//			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			...
			for (SettleTransSummary summaryBean : summaryList) {
				...
				try {
					detailBean = ((DsfMessageListener) AopContext.currentProxy()).updateSummaryAndDetailOne(
							summaryBean
							, dflag
							, errorMsg
				...
				if (((hour == 18 && minute >= 30) || hour > 18) 
						|| (dflag == 6 && detailBean != null)) {// 18:30之后成功失败的都立即发给订单或者18:30之前的成功的立即发给订单
					// sendToOrderList.add(detailBean);
					try {
						/*
						 * if(detailBean.getDflag() == 4){ //true=冲正 else=抹账
						 * if(detailBean
						 * .getAccountDate().equals(sdf.format(settleDate))){
						 * czList.add(detailBean); }else{
			...
			/*
			 * if(czList !=null && czList.size()>0){ doCorrectByAccount(czList);
			 * } if(mzList !=null && mzList.size()>0){
			 * doAccOfOrRefundByAccount(mzList); }
			 */
		} catch (Exception e) {
			logger.error(">>>>>将代收付结果更新到汇总表updateSummeryStatusId发生异常!!!!!!batchNo=" + batchNo, e);
		}
		return rtnMap;
	}
	...
	public SettleTransDetail updateSummaryAndDetailOne(
		...
		try {
			...
			if (detailList != null && detailList.size() > 0) {
				...
				} else if (detailBean.getDflag() == 6
						|| detailBean.getDflag() == 60) {// 成功
			...
			detailBean = null;
			logger.error("代收付的MQ监听类中更新单条汇总表和交易表发生异常!!!!!!orderNo="
	...
	public void removeSuccessToHis() {
		try {
			...
			if ((hour == 18 && minute >= 30) || hour > 18) {// 18:30之后成功失败的都立即发给订单或者18:30之前的成功的立即发给订单
				...
				logger.info("18:30之后,将【失败】单移入历史表完成");
				map.put("iv_status_id", "15");
				super.getSqlSession().selectOne("SettleTransInvoiceMapper.setgeneration", map);
				logger.info("18:30之后,将【成功】单移入历史表完成");
			} else {
				map.put("iv_status_id", "15");
				super.getSqlSession().selectOne("SettleTransInvoiceMapper.setgeneration", map);
				logger.info("18:30之前,将【成功】单移入历史表完成");
			}
		} catch (Exception e) {
			logger.error(">>>>>removeSuccessToHis将成功的代收付移到历史表异常！！！！！！"+ e.getMessage(), e);
		}
	}
	...
	public void returnToOrder(List<SettleTransDetail> sendToOrderList) {
		...
		try {
			if (orderPaymentList != null && orderPaymentList.size() > 0) {
				resultmap = settlementToOrderService
	 ...
	 * @return
	 */
	public void doAccOfOrRefundByAccount(
			List<SettleTransDetail> settleTransDetailList) throws Exception {
...
//			String funcCode = settleTransDetail.getFuncCode();
			...
			try {
				...
				// payIntOutSer.wipeAccountForDubbo(newOrderNo, orderNo,
				// merchantCode);
				...
				logger.info("调用[mtaegis抹帐接口]返回值: {code:" + code + ", msg:"
						+ msg + "}");
				...
				settleTransDetailManager
						.updateSettleTransDetail(settleTransDetail);
			} catch (Exception e) {
	 ...
	 * @return
	 */
	public void doCorrectByAccount(List<SettleTransDetail> settleTransDetailList)
			throws Exception {
//		Map<String, Object> resultMap = new HashMap<String, Object>();
		Iterator<SettleTransDetail> settleTransDetailIter = settleTransDetailList
				.iterator();
		...
		while (settleTransDetailIter.hasNext()) {
			/**
			 * (商户扣款冲正dubbo提供接口)
			 * @Params : funcCode :交易编码 商户冲正 10011 orderNo : 新订单号 userIpAddress
			 *         : 用户IP地址 orderPackageNo : 订单包号 ----原定单号 rootInstCd : 机构码
			 * @Return :
			...
			try {
				// ErrorResponse errorResponse =
				// payAccSerApi.antideductForDubbo(funcCode, newOrderNo,
				// userIpAddress, orderNo, rootInstCd);
				// boolean isSuccess = errorResponse.isIs_success();
				transOrderInfo = new TransOrderInfo();
				transOrderInfo.setOrderNo(newOrderNo);
				transOrderInfo.setOriginalOrderId(orderNo);
	 ...
	 * @return
	 */
	public String getOrgNumAndChannelHome(String orgCode) throws Exception {
		logger.info(">>> >>> 开始:通过orgCode查多渠道的ORG_INFO表");
		String OrgNumAndChannelHome = null;
		OrgInfoQuery query = new OrgInfoQuery();
		query.setOrgCode(orgCode);
		List<OrgInfo> list = orgInfoManager.queryList(query);
		if (list == null || list.size() < 1) {
			logger.error(">>> >>> 异常:通过orgCode查多渠道的ORG_INFO表 查询结果为0条, return orgCode:"
					+ orgCode);
			return orgCode;
		}
		OrgInfo orgInfo = list.get(0);
		OrgNumAndChannelHome = orgInfo.getOrgNum() + ","
				+ orgInfo.getChannelHome();
		logger.info("<<< <<< 结束:通过orgCode查多渠道的ORG_INFO表");
	 ...
	 */
	public void splitAccountData(
			Map<String, List<SettleTransDetail>> detailList,
			List<com.rkylin.wheatfield.bean.TransOrderInfo> param1List,
			List<TransOrderInfo> param2DfList, List<TransOrderInfo> param2DsList) {
		try {
			if (detailList == null || detailList.size() == 0) {
			...
			if (detailList.containsKey("1")) { // 账户1期数据
				...
				for (SettleTransDetail detailBean : subList) {
					...
					transOrderInfoOne.setMerchantCode(detailBean
							.getMerchantCode());
					...
					if (detailBean.getDflag() == 4
						...
						transOrderInfoOne.setErrorCode(detailBean
								.getErrorCode());
						...
						logger.info(detailBean.getTransDetailId()
								+ "一期代付结果状态异常！");
						continue;
					}
					transOrderInfoOne.setRemark(String.valueOf(detailBean
							.getTransDetailId()));
					logger.info("账户一期接口的请求内部参数：订单号="
							+ transOrderInfoOne.getOrderNo() + ",金额="
							+ transOrderInfoOne.getAmount());
			...
			if (detailList.containsKey("2")) { // 账户2期数据
				...
				for (SettleTransDetail detailBean : subList) {
					transOrderInfo = new TransOrderInfo();
					transOrderInfo
							.setOrderDate(settlementUtil.getCurrentTime());
					transOrderInfo.setOrderNo(settlementUtil
							.createBatchNo("ST"));
					...
					transOrderInfo.setUserIpAddress(detailBean
							.getUserIpAddress());
					transOrderInfo.setRemark(String.valueOf(detailBean
							.getTransDetailId()));

					if (SettleConstants.BATCH_DF_FUN_CODE.equals(detailBean
							.getFuncCode())
							|| SettleConstants.BATCH_TX_FUN_CODE
									.equals(detailBean.getFuncCode())
							|| SettleConstants.BATCH_DF_YFQ_FUN_CODE
									.equals(detailBean.getFuncCode())) {
						transOrderInfo.setDealProductCode("MPROD_00_RS_001");
						logger.info("二期，订单号=" + transOrderInfo.getOrderNo()
								+ ",原始订单号："
								+ transOrderInfo.getOriginalOrderId() + "订单包号："
								+ transOrderInfo.getOrderPackageNo());
						param2DfList.add(transOrderInfo);
					} else if (SettleConstants.BATCH_DS_FUN_CODE
							.equals(detailBean.getFuncCode())) {
						...
						logger.info("二期，原始订单号：="
								+ transOrderInfo.getOriginalOrderId()
								+ "的funcCode不是代收付业务的funcCode");
					}
				}
			}
		} catch (Exception e) {
	 ...
	 * @param paramList
	 */
	public void dealDsfResultTo1Account(
			List<com.rkylin.wheatfield.bean.TransOrderInfo> param1List) {
		try {
			int countList = 0;
			...
			if (param1List != null && param1List.size() > 0) {
				...
				while (totalIndex >= batchIndex) {
					try {
						...
						toIndex = totalIndex == batchIndex ? countList
								: batchIndex * size;
						paramTmpList = param1List.subList(fromIndex, toIndex);
						if (paramTmpList == null) {
							continue;
						}
						logger.info("一期数据调用账户系统代收付结果推送接口，共推送"
								+ paramTmpList.size() + "条，开始");

						logger.info("transOrderDubboService="
								+ transOrderDubboService);
						if (paramTmpList.size() > 0) {
							logger.info("参数:merchantCode="
									+ paramTmpList.get(0).getMerchantCode()
									+ ";orderNo="
									+ paramTmpList.get(0).getOrderNo()
									+ ";amount="
									+ paramTmpList.get(0).getAmount());
						}
						rtnResponse = transOrderDubboService
								.notifyTransOrderResults(paramTmpList);// 代付结果推送给账户
						logger.info("一期数据调用账户系统代收付结果推送接口，共推送"
								+ paramTmpList.size() + "条，结束");
						if (rtnResponse != null && !"".equals(rtnResponse)) {
							if ("1".equals(rtnResponse.getCode())) {
								logger.info("一期数据调用账户系统代收付结果推送接口，共推送"
										+ paramTmpList.size() + "条，账户系统返回成功！");
	...
	public void dealDsResultTo2Account(List<TransOrderInfo> param2DsList) {
		try {
			...
			if (param2DsList != null && param2DsList.size() > 0) {
				...
				while (totalIndex >= batchIndex) {
					try {
						String orderPackageNo = settlementUtil
								.createBatchNo("SP");
						fromIndex = (batchIndex - 1) * size;
						toIndex = totalIndex == batchIndex ? countList
								: batchIndex * size;
						...
						while (ite.hasNext()) {
							...
							tmpTransOrderInfo
									.setOrderCount(paramTmpList.size());
							tmpTransOrderInfo.setOrderPackageNo(orderPackageNo);
							logger.info("二期，订单号="
									+ tmpTransOrderInfo.getOrderNo()
									+ ",原始订单号："
									+ tmpTransOrderInfo.getOriginalOrderId()
									+ "订单包号："
									+ tmpTransOrderInfo.getOrderPackageNo());
						}
						logger.info("二期代收结果推送给账户系统，开始》》》》》》,paramRList.size="
								+ paramTmpList.size());
						rtnDsResponse = multCollectionResultReturnApi
								.execute(paramTmpList);// 代收结果推送给账户
						logger.info("二期代收结果推送给账户系统，结束》》》》》》,paramRList.size="
								+ paramTmpList.size());
						if (rtnDsResponse != null && !"".equals(rtnDsResponse)) {
							logger.info("二期代付结果推送给账户系统,账户系统返回的code="
									+ rtnDsResponse.getCode());
							if (!"1".equals(rtnDsResponse.getCode())) {
								rtnDsList = rtnDsResponse.getErrList();
								if (rtnDsList != null && rtnDsList.size() > 0) {
									this.updateDflagByAccountFail(paramTmpList,
											rtnDsList);// 部分失败
									logger.info("二期代收结果推送给账户系统，账户系统有部分处理失败，共"
											+ rtnDsList.size());
								} else {
									this.updateDflagByAccountFail(paramTmpList,
											null);// 全部失败
			...
			if (param2DfList != null && param2DfList.size() > 0) {
				...
				while (totalIndex >= batchIndex) {
					try {
						...
						logger.info("二期代付结果推送给账户系统，开始》》》》》》,paramPList.size="
								+ paramTmpList.size());
						rtnDfResponse = multWithholdAndWithdrowResultReturnApi
								.execute(paramTmpList);// 代付结果推送给账户
						logger.info("二期代付结果推送给账户系统，结束》》》》》》,paramPList.size="
								+ paramTmpList.size());
						if (rtnDfResponse != null && !"".equals(rtnDfResponse)) {
							logger.info("二期代付结果推送给账户系统,账户系统返回的code="
									+ rtnDfResponse.getCode());
							if (!"1".equals(rtnDfResponse.getCode())) {
								rtnDfList = rtnDfResponse.getErrList();
								if (rtnDfList != null && rtnDfList.size() > 0) {
									logger.info("二期代付结果推送给账户系统，账户系统有部分处理失败，共"
											+ rtnDfList.size());
									this.updateDflagByAccountFail(paramTmpList,
											rtnDfList);// 部分失败
								} else {
	...
	public void updateSend1AccountFail(
		...
		try {
			...
			for (com.rkylin.wheatfield.bean.TransOrderInfo transBean : paramTmpList) {
				...
				detailBean.setTransDetailId(Integer.parseInt(transBean
						.getRemark()));
				...
				logger.info("准备更新detail表 detailId="
						+ detailBean.getTransDetailId() + ";dflag="
						+ detailBean.getDflag());
				settleTransDetailManager.updateSettleTransDetail(detailBean);
			}
			detailBean = null;
	 ...
	 * @param paramList
	 */
	public void updateDflagByAccountFail(List<TransOrderInfo> paramList,
			List<MultTransOrderInfoReturn> rtnList) {
		try {
			SettleTransDetail detail = new SettleTransDetail();
			for (TransOrderInfo transBean : paramList) {
				detail.setTransDetailId(Integer.parseInt(transBean.getRemark()));
				if (rtnList != null && rtnList.size() != 0) {// 部分失败
					for (MultTransOrderInfoReturn bean : rtnList) {
						logger.info("orderNo=" + bean.getOriginalOrderId()
								+ ",二期账户系统返回失败");
						if (transBean.getOriginalOrderId().compareTo(
								bean.getOriginalOrderId()) == 0) {
							if (transBean.getStatusId() == 5) {
								detail.setDflag(40);
							} else if (transBean.getStatusId() == 4) {
								detail.setDflag(60);
