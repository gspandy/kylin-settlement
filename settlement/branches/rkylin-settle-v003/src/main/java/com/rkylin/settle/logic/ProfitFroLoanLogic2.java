...
public class ProfitFroLoanLogic2 extends BasicLogic {
...
//		String profitId = "";
	...
	public Map<String, Object> doInvoiceSettle(List<SettleProfitInvoice> settleProfitInvoiceList) {
		...
		while (iter.hasNext()) {
			...
			transorderinfo.setInterUserId(item.getInterMerchantCode());
			transorderinfo.setRootInstCd(item.getMerchantCode());
...
//	}
	...
	private Map<String, String> toAccWithhold(TransOrderInfo transorderinfo) {
		...
		acRequest.setIntermerchantcode(transorderinfo.getIntoRootInstCd());
		acRequest.setMerchantcode(transorderinfo.getRootInstCd());
...
//		acRequest.setFeeamount(transorderinfo.getFeeAmount());
	...
	private Map<String, String> toAccWithdrow(TransOrderInfo transorderinfo) {
		...
		acRequest.setOrdercount(transorderinfo.getOrderCount());
		acRequest.setFunccode(transorderinfo.getFuncCode());
		acRequest.setIntermerchantcode(transorderinfo.getInterUserId());
		acRequest.setMerchantcode(transorderinfo.getRootInstCd());
		acRequest.setUserid(transorderinfo.getUserId());
		acRequest.setAmount(transorderinfo.getAmount());
	...
	private TransOrderInfo createTransOrderInfo(SettleTransBill bill,
		...
		transOrderInfo.setAmount(transferAmount);
		transOrderInfo.setUserId(bill.getUserId());
