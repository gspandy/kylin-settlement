...
//			query.setAccountDate(item.getAccountDate());
			...
			if(count > 0) {//存在
				...
				updateItem.setRelateOrderNo(item.getRelateOrderNo());
...
//					throw new SettleException("'上游or清算的对账信息'的Map中,key'"+ key +"'所对应的value为'null'");
	...
	protected Map<String, Object> changeAccount(String instCode, String userId, String productId, long amount, String funcCode, String orderNo, String remark) throws Exception {
		logger.info(">>> >>> >>> 开始:调用账户调账接口 账户一期[改]");
		...
		rtnMap.put("code", commonResponse.code);
		rtnMap.put("msg", commonResponse.msg);
		logger.info("<<< <<< <<< 结束:调用账户调账接口 账户一期[改]");
		return rtnMap;
	}
	/**
	 * 调用账户调账接口 账户二期
...
//		rtnMap.put("msg", commonResponse.msg);
	 ...
	 * @throws Exception
	 */
	public void sendMail(String subject, String content, String mailType,File file) throws Exception {
		
		SettleParameterInfoQuery query = new SettleParameterInfoQuery();
		//参与分润的功能编码
		String parameterType = SettleConstants.PARAMETER_TYPE_MAIL_TO;
		//查询parameter表的query对像
		query.setParameterType(parameterType);
		query.setParameterCode(mailType);
		query.setStatusId(1);
		//查询 mail的To对象
		List<SettleParameterInfo> parameterInfoList = settleParameterInfoManager.queryList(query);
		
		if(parameterInfoList == null || parameterInfoList.size()==0){
			logger.info("查询 mail的To对象为空");
			return;
		}
		
		String[] to = parameterInfoList.get(0).getParameterValue().split(SettleConstants.DEDT_SPLIT2);
		
		parameterType = SettleConstants.PARAMETER_TYPE_MAIL_CC;
		//查询parameter表的query对像
		query.setParameterType(parameterType);
		query.setParameterCode(mailType);
		query.setStatusId(1);
		...
		if(parameterInfoList == null || parameterInfoList.size()==0){
			logger.info("查询 mail的Cc对象为空");
			return;
		}

		String[] cc = parameterInfoList.get(0).getParameterValue().split(SettleConstants.DEDT_SPLIT2);
		
		if (file == null) {
			RkylinMailUtil.sendMail(subject, content, to, cc);
		} else {
			RkylinMailUtil.sendAttachmentMail(subject, content, file, to, cc);
		}
	}
	/**
	 * 
	 * 创建SettleKernelEntry信息
	 * @param transId			全局流水号
	 * @param settleNo			全局流水号String
	 * @param transNo			流水号
	 * @param kernelFuncCode	会计功能编码 例如:F5010
	 * @param finAccountId1		借记账户ID
	 * @param finAccountId2		贷记账户ID
	 * @param paymentAmout		发生额
	 * @param referEntryId		全局流水号
	 * @param accountDate		账期
	 * @param collectType		结算类型
	 * @param rootInstCd		机构号
	 * @param settleDay			结算日期
	 * @param actualSettleDay	实际结算日期
	 * @return
	 */
	protected SettleKernelEntry createSettleKernelEntry(
			Long transId,
			String settleNo,
			Integer transNo, 
			String kernelFuncCode, 
			String finAccountId1,  
			String finAccountId2, 
			Long paymentAmout, 
			String referEntryId, 
			Date accountDate, 
			String collectType, 
		...
		return this.createSettleKernelEntry(transId, settleNo, transNo, kernelFuncCode, finAccountId1, finAccountId2, paymentAmout, referEntryId, accountDate, collectType, rootInstCd, accountDate, accountDate);
	}
	/**
	 * 
	 * 创建SettleKernelEntry信息
	 ...
	 * @return
	 */
//	private SettleKernelEntry createSettleKernelEntry(
//			Long transId,
//			String settleNo,
//			Integer transNo, 
//			String kernelFuncCode, 
...
//			String rootInstCd
//			) {
//		return this.createSettleKernelEntry(transId, settleNo, transNo, kernelFuncCode, finAccountId1, finAccountId2, paymentAmout, referEntryId, accountDate, collectType, rootInstCd, accountDate, actualSettleDate);
//	}
	/**
	 * 
	 * 创建SettleKernelEntry信息
	...
	private SettleKernelEntry createSettleKernelEntry(
		...
		logger.info(">>> >>> 开始 创建SettleKernelEntry信息");
		SettleKernelEntry settleKernelEntry = new SettleKernelEntry();
		settleKernelEntry.setTransId(transId);
		settleKernelEntry.setSettleNo(settleNo);
		settleKernelEntry.setTransDate(transDate);
		settleKernelEntry.setRequestIdFrom("3");
		settleKernelEntry.setTransNumber(1);
		settleKernelEntry.setTransNo(transNo);
		settleKernelEntry.setFuncCode(kernelFuncCode);
		settleKernelEntry.setFinAccountId1(finAccountId1);
		settleKernelEntry.setFinAccountId2(finAccountId2);
		settleKernelEntry.setPaymentAmount1(paymentAmount);
		settleKernelEntry.setPaymentAmount2(paymentAmount);
		settleKernelEntry.setCurrency1("CNY");
		settleKernelEntry.setCurrency2("CNY");
		settleKernelEntry.setReferEntryId(referEntryId);
		settleKernelEntry.setAccountDate(accountDate);
		settleKernelEntry.setReadStatusId(1);
		settleKernelEntry.setRemark1(collectType);
		settleKernelEntry.setRootInstCd(rootInstCd);
		settleKernelEntry.setSettleCycle("1");
		settleKernelEntry.setTn("1");
		settleKernelEntry.setSettleDay(settleDay);
		settleKernelEntry.setActualSettleDay(new Date());
		logger.info("<<< <<< 结束 创建SettleKernelEntry信息");
		return settleKernelEntry;
	}
	/**
	 * 创建TransEntrySa信息
	 * @param transId				SETTLE_KERNEL_ENTRY.ID
	 * @param settleKernelEntry		汇总信息
	 * @return
	 */
	protected TransEntrySa createTransEntrySa(Long transId, SettleKernelEntry settleKernelEntry) {
		logger.info(">>> >>> 开始 创建TransEntrySa信息");
		TransEntrySa transEntrySa = new TransEntrySa();
		transEntrySa.setTransDate(settleKernelEntry.getTransDate());
		transEntrySa.setRequestIdFrom(settleKernelEntry.getRequestIdFrom());
		transEntrySa.setTransId(settleKernelEntry.getTransId());
		transEntrySa.setTransNumber(settleKernelEntry.getTransNumber());
		transEntrySa.setTransNo(settleKernelEntry.getTransNo());
		transEntrySa.setFuncCode(settleKernelEntry.getFuncCode());
		transEntrySa.setFinAccountId1(settleKernelEntry.getFinAccountId1());
		transEntrySa.setFinAccountId2(settleKernelEntry.getFinAccountId2());
		transEntrySa.setPaymentAmout1(settleKernelEntry.getPaymentAmount1());
		transEntrySa.setPaymentAmout2(settleKernelEntry.getPaymentAmount2());
		transEntrySa.setCurrency1(settleKernelEntry.getCurrency1());
		transEntrySa.setCurrency2(settleKernelEntry.getCurrency2());
		transEntrySa.setReferEntryId(settleKernelEntry.getReferEntryId());
		transEntrySa.setAccountDate(settleKernelEntry.getAccountDate());
		transEntrySa.setStatusId(settleKernelEntry.getReadStatusId());
		transEntrySa.setRemark(settleKernelEntry.getRemark1());
		logger.info("<<< <<< 结束 创建TransEntrySa信息");
		return transEntrySa;
	}
	/**
	 *  创建TransEntrySa信息
	 ...
	 * @return
	 */
//	private TransEntrySa createTransEntrySa(Long transId, Integer transNo, String kernelFuncCode, String finAccountId1,  String finAccountId2, Long paymentAmout, String referEntryId, Date accountDate, String collectType) {
//		TransEntrySa transEntrySa = new TransEntrySa();
//		transEntrySa.setTransDate(new Date());
//		transEntrySa.setRequestIdFrom("3");
//		transEntrySa.setTransId(transId);
//		transEntrySa.setTransNumber(1);
//		transEntrySa.setTransNo(transNo);
//		transEntrySa.setFuncCode(kernelFuncCode);
//		transEntrySa.setFinAccountId1(finAccountId1);
//		transEntrySa.setFinAccountId2(finAccountId2);
//		transEntrySa.setPaymentAmout1(paymentAmout);
//		transEntrySa.setPaymentAmout2(paymentAmout);
//		transEntrySa.setCurrency1("CNY");
//		transEntrySa.setCurrency2("CNY");
//		transEntrySa.setReferEntryId(referEntryId);
//		transEntrySa.setAccountDate(accountDate);
//		transEntrySa.setStatusId(1);
//		transEntrySa.setRemark(collectType);
//		return transEntrySa;
//	}
	/**
	 * 创建RegisterFalut信息
	 * @param settleNo			SETTLE_KERNEL_ENTRY.ID
	 * @param settleKernelEntry	汇总信息结构体
	 * @return
	 */
//	private RegisterFalut createRegisterFalut(String settleNo, SettleKernelEntry settleKernelEntry) {
//		logger.info(">>> >>> >>> 开始  创建RegisterFalut信息");
//		RegisterFalut registerFalut = new RegisterFalut();
//		registerFalut.setRootInstCd(settleKernelEntry.getRootInstCd());
//		registerFalut.setSettleCycle(settleKernelEntry.getSettleCycle());
//		registerFalut.setTn(settleKernelEntry.getTn());
//		registerFalut.setSettleDay(settleKernelEntry.getSettleDay());
//		registerFalut.setActualSettleDay(settleKernelEntry.getActualSettleDay());
//		registerFalut.setSettleNo(settleNo);
//		logger.info("<<< <<< <<<< 结束  创建RegisterFalut信息");
//		return registerFalut;
//	}
	/**
	 * 创建RegisterFalut信息
	 * @param rootInstCd	机构号
	 * @param collectType	对账类型
	 * @param accountDate	账期
	 * @param settleNo		SETTLE_KERNEL_ENTRY.ID
	 * @return
	 */
//	private RegisterFalut createRegisterFalut(String rootInstCd, String collectType, Date accountDate, String settleNo) {
//		return this.createRegisterFalut(rootInstCd, collectType, accountDate, accountDate, settleNo);
//	}
	/**
	 * 创建RegisterFalut信息
	 * @param rootInstCd		机构号
	 * @param collectType		对账类型
	 * @param settleDay			结算日
	 * @param actualSettleDay	实际结算日
	 * @param settleNo			SETTLE_KERNEL_ENTRY.ID
	 * @return
	 */
//	private RegisterFalut createRegisterFalut(String rootInstCd, String collectType, Date settleDay, Date actualSettleDay, String settleNo) {
//		RegisterFalut registerFalut = new RegisterFalut();
//		registerFalut.setRootInstCd(rootInstCd);
//		registerFalut.setSettleCycle("1");
//		registerFalut.setTn("1");
//		registerFalut.setSettleDay(settleDay);
//		registerFalut.setActualSettleDay(actualSettleDay);
//		registerFalut.setSettleNo(settleNo);
//		return registerFalut;
//	}
	/**
	 ...
	 * @return
	 */
	protected Long genTimestampAndRandom() {
		Long time = (new Date()).getTime();
		Long random = Long.valueOf(String.valueOf((long)(Math.floor(Math.random() * 1000))));
		Long theNum = Long.valueOf(time + "" + random);
		return theNum;
	}
	/**
	 * 将用','分个的主键字符串编辑成Long[]主键数组
	 * @param idStr
	 * @return
	 */
	protected Long[] editTransDetailIdArr(String idStr) {
		String[] idStrArr = idStr.split(",");
		Long[] idArr = new Long[idStrArr.length];
		for(int i = 0; i < idStrArr.length; i ++) {
			idArr[i] = Long.valueOf(idStrArr[i]);
		}
		return idArr;
	}
	/**
	 * 持久化汇总信息
	 * @param settleKernelEntryList
	 */
	protected void insertAndUpdateSettleKernelEntry(List<SettleKernelEntry> settleKernelEntryList) {
		logger.info(">>> >>> >>> 更新'交易结果'文件 ... ...");
		//对账信息查询query对象
		if (settleKernelEntryList.size() > 0) {
			for (SettleKernelEntry ske : settleKernelEntryList) {
				if(ske.getId() == null) {
					settleKernelEntryManager.saveSettleKernelEntry(ske);
				} else {
					settleKernelEntryManager.updateSettleKernelEntry(ske);
				}
			}
		}
	}
