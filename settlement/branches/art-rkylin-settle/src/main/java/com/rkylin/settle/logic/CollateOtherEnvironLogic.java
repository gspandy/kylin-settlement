 ...
 * @author Yang
 */
@Component("CollateOtherEnvironLogic")
public class CollateOtherEnvironLogic extends BasicLogic {
	@Autowired
	...
	@Autowired
	private SettleTransDetailOtherEnvironManager settleTransDetailOEManager;
	@Autowired
	private SettleBalanceEntryManager settleBalanceEntryManager;	//对账结果Manager
	
	private String otherEnviron;//其他环境名称
	/**
	 * 进入其他环境对账
	 * @param tAccountList 上游数据
	 * @param tDetailList 交易数据
	 * @param otherEnviron 对账环境
	 * @param rule 对账规则参数（01，02）
	 * @return
	 * @throws Exception
	 */
	public synchronized Map<String, Object> doCollate(List<Map<String, Object>> tAccountList, List<Map<String, Object>> tDetailList,String otherEnviron,String rule) throws Exception {
		logger.info(">>> >>> >>> 进入其他环境对账（"+otherEnviron+"环境，rule:"+rule+"） ... ...");
		this.otherEnviron = otherEnviron;
		//提示信息
		Map<String, Object> resultMap = new HashMap<String, Object>();
		//对账结果List, 暂存对账结果信息
		List<Map<String, Object>> settleBalanceEntryList = new ArrayList<Map<String, Object>>();
		//错账订单号
		...
		//对账状态列表, 用于 update ‘上游渠道’对账信息
		//List<SettleTransAccount> accInfoList = new ArrayList<SettleTransAccount>();
		...
		//声明对账结果对象
		Map<String, Object> settleBalanceEntry = null;
		try {
			//获取对账规则, 通过上游渠道ID获取对账规则
			keysAndParams = this.getRule("OE01", rule);
		...
		System.out.println("***accountMapStru**************************************************");
		System.out.println(accountMapStru.size());
		System.out.println(accountMapStru);
		System.out.println("***detailMapStru**************************************************");
		System.out.println(detailMapStru.size());
		System.out.println(detailMapStru);
		System.out.println("*****************************************************");
		logger.info(">>> >>> >>> rule:"+rule+"，对账开始 ... ...");
		for(String ruleKey : detailMapStru.keySet()) {//遍历'清算'方对账结构体
			//实例化对账结果对象
			settleBalanceEntry = new HashMap<String, Object>();
			...
			//外部对账1, 内部对账账户&多渠道2
			settleBalanceEntry.put("balanceType", 1);
			...
			if(accountMapStru.containsKey(ruleKey)) {//[我有, 他也有]
				...
				//
				settleBalanceEntry.put("orderNo", orderNo);
				settleBalanceEntry.put("obligate1", ruleKey);
				settleBalanceEntry.put("obligate2", detIdAndAccId);
				settleBalanceEntry.put("accId", "#"+accountMap.get("TRANS_ACCOUNT_ID"));
				
				logger.info(">>> >>> 生效[orderNo:"+ ruleKey +"],detIdAndAccId："+detIdAndAccId+",orderNo:"+orderNo);
				...
				//TODO 对平：该长款是其他环境来的
				if(allRight) {//平账
					logger.info(">>>  对平：长款来自"+otherEnviron+"环境 [orderNo:"+ ruleKey +"]");
					settleBalanceEntry.put("statusId", SettleConstants.COLLATE_STU_4);
					settleBalanceEntry.put("remark", "长款："+otherEnviron+"环境(2-4)");
					...
					colStatInfoMap.put("remark", "长款："+otherEnviron+"环境(由原：23，改为现：211)");
					colStatInfoMap.put("statusId", SettleConstants.STATUS_COLLATE_OE_SUCCESS);
					colStatInfoList.add(colStatInfoMap);
					
					
				} else {//TODO 错账：对账的校验金额出现问题
					failParams = failParams.substring(1);//干掉拼串
					logger.info(">>>  错账("+otherEnviron+"环境): [orderNo:"+ ruleKey +"]");
					failOrderNo.add(ruleKey);
					failOrderMsg.put(ruleKey,"错账项目:" + "错账("+otherEnviron+"环境), 未平项目:" + failParams);
					settleBalanceEntry.put("statusId", SettleConstants.COLLATE_STU_0);
					settleBalanceEntry.put("remark", "错账("+otherEnviron+"环境): 未平项目:" + failParams);
					
					//用于 update ‘上游’表
					Map<String, Object> colStatInfoMap = new HashMap<String, Object>();
					//colStatInfoMap.put("detId", detailMap.get("TRANS_DETAIL_ID"));
					colStatInfoMap.put("accId", accountMap.get("TRANS_ACCOUNT_ID"));
					colStatInfoMap.put("remark", "错账："+otherEnviron+"环境");
				...
				//记录'清算'账期
				settleBalanceEntry.put("accountDate", (Date)detailMap.get("ACCOUNT_DATE"));
				//填入对账结果List
				settleBalanceEntryList.add(settleBalanceEntry);
			}
		}
		logger.info(">>> >>> >>> rule:"+rule+"，对账结束 ... ...");
		 ...
		 * 所以,做如下处理.
		 */
		this.updateSettleOtherEnvironBalanceEntry(settleBalanceEntryList);
		/**
		 * 更新'清算'交易信息状态
		 * 更新'上游渠道'交易信息状态
		 */
		this.updateAccOtherEnviron(colStatInfoList);
		
		//封装返回值对象
		if(failOrderNo.size() > 0) {//有未对平的订单被记录
			this.editResultMap(resultMap, "2", otherEnviron+"环境对账执行结束, 有["+failOrderNo.size()+"]条账目未对平", failOrderNo, failOrderMsg);
			logger.info(">>> >>> "+otherEnviron+"环境对账执行结束, 有["+failOrderNo.size()+"]条账目未对平, 详情请看本方法的返回结果或查看db对账结果中REMARK信息");
		} else {//爽歪歪, 全都对平!
			this.editResultMap(resultMap, "1", otherEnviron+"环境对账结束, 全都对平!");
			logger.info(">>> >>> "+otherEnviron+"环境对账结束, 全都对平!");
		}
		
		logger.info(">>> >>> >>> >>> 执行结束  其他环境对账，rule:"+rule+" ... ...");
		return resultMap;
	}
	/**
	 * 从多渠道读取交易信息并存入'清算'DB
...
//			}
	...
	/***
	 * TODO 更新对账结果表（其他环境对账后）
	 * @param settleBalanceEntryList
	 * 对账结果List, 暂存对账结果信息
	 */
	protected void updateSettleOtherEnvironBalanceEntry(List<Map<String,Object>> settleBalanceEntryList) {
		logger.info(">>> >>> >>> （其他环境对账后）更新对账结果表 （"+otherEnviron+"环境）开始... ...");
		if (settleBalanceEntryList.size() > 0) {
			for (Map<String,Object> subbean : settleBalanceEntryList) {
				settleBalanceEntryManager.updateByAccIdSelective(subbean);
			}
		}
		logger.info(">>> >>> >>> （其他环境对账后）更新对账结果表 （"+otherEnviron+"环境）完成... ...");
	}
	
	/*** 
	 * TODO 更新上游交易状态和备注信息（其他环境对账后）
	 * @param colStatInfoList 
	 */
	private void updateAccOtherEnviron(List<Map<String, Object>> colStatInfoList) {
		logger.info(">>> >>> >>> （其他环境对账后）更新上游交易状态和备注信息（其他环境对账后） （"+otherEnviron+"环境）开始... ...");
		...
		while(iter.hasNext()) {
			...
			if(SettleConstants.STATUS_COLLATE_OE_SUCCESS == Integer.parseInt(statusId)) {
		...
		if(succAccIdList.size() > 0) {
			...
			map.put("statusId", SettleConstants.STATUS_COLLATE_OE_SUCCESS);
			map.put("remark", "长款："+otherEnviron+"环境(由原：23，改为现：211)");
		...
		if(errAccIdList.size() > 0) {
			...
			map.put("remark", "错款："+otherEnviron+"环境");
			map.put("idList", errAccIdList);
			settleTransAccountManager.updateTransStatusId(map);
		}
		logger.info(">>> >>> >>> （其他环境对账后）更新上游交易状态和备注信息（其他环境对账后） （"+otherEnviron+"环境）完成... ...");
	}
	/***
	 * 更新'上游渠道'和'清算'的交易状态
	 * @param colStatInfoList 
	 ...
	 * @return
	 */
	public List<Map<String, Object>> querySettleTransDetail(Date accountDate) throws Exception {
		/*
		 * 查询清算交易Query对象
		 */
		...
		return settleTransDetailOEManager.queryListMapOE(query);
	}
	/***
	 * 上游渠道交易表的数据取得
	 * 亮哥
	 ...
	 * @return
	 */
	public List<Map<String, Object>> querySettleTransAccount(Date settleTime) throws Exception {
		SettleTransAccountQuery query = new SettleTransAccountQuery();
		query.setSettleTime(settleTime);//时间
		//query.setRemark("");//备注为长款
		query.setStatusId(23);//只查询长款
