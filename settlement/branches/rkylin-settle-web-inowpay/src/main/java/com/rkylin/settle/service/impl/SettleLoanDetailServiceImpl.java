 ...
 *
 */
@Service("settleLoanDetailService")
public class SettleLoanDetailServiceImpl implements SettleLoanDetailService {
	private static Logger logger = LoggerFactory.getLogger(SettleLoanDetailServiceImpl.class);
	@Autowired
	...
	public PagerModel<SettleLoanDetail> query(SettleLoanDetailQuery query) {
		//创建分页Model
		PagerModel<SettleLoanDetail> pagerModel = new PagerModel<SettleLoanDetail>();
		try {
			//初始化当前页
			...
			pagerModel.setList(settleLoanDetailManager.queryPage(query));
			pagerModel.setTotal(settleLoanDetailManager.countByExample(query));
	...
	public SettleLoanDetail findById(Long id) {
		return settleLoanDetailManager.findSettleLoanDetailById(id);
	}
	/***
	 * 修改下游对账交易信息
	 * @param query
	...
	public Integer edit(SettleLoanDetail settleLoanDetail) {
		return settleLoanDetailManager.updateSettleLoanDetail(settleLoanDetail);
	}
	/***
	 * 批量查询交易信息by主键数组
	 * @param queryMap
	...
	public Map<String, Object> doCorrectByAccount(List<SettleTransDetail> settleTransDetailList) throws Exception {
		...
		while(settleTransDetailIter.hasNext()) {
			...
			String rootInstCd = settleTransDetail.getMerchantCode();
			try {
	...
	private TransOrderInfo getTransOrderInfoBySettleTransDetail(SettleTransDetail settleTransDetail) {
		 ...
		 * 入钱方
		 */
		transOrderInfo.setInterMerchantCode(settleTransDetail.getUserId());
		transOrderInfo.setMerchantCode(settleTransDetail.getMerchantCode());
		transOrderInfo.setAmount(settleTransDetail.getAmount());
		transOrderInfo.setFeeAmount(settleTransDetail.getFeeAmount());
		...
		transOrderInfo.setErrorCode(settleTransDetail.getErrorCode());
		transOrderInfo.setErrorMsg(settleTransDetail.getErrorMsg());
		if(settleTransDetail.getCancelInd() == 1) transOrderInfo.setStatus(6);
		transOrderInfo.setStatus(settleTransDetail.getReadStatusId());
		transOrderInfo.setRemark(settleTransDetail.getRemark());
		transOrderInfo.setAccountDate(settleTransDetail.getAccountDate());
