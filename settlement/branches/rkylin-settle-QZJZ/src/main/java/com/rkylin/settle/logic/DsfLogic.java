...
//		configMap.put("multiSheet_index", "0");
		...
		*/
		...
		if(settleTransInvoiceList != null && settleTransInvoiceList.size() > 0) {
			for(int i = 0; i < settleTransInvoiceList.size(); i ++) {
				...
				detailMap.put("F_5", sti.getBankCode());//收款银行列
				detailMap.put("F_6", sti.getOpenBankName());//收款银行支行列
				detailMap.put("F_7", sti.getProvince());//收款省/直辖市列
				detailMap.put("F_8", sti.getCity());//收款市县列
		...
		String nwDate = sdf.format(new Date());
		...
		RkylinMailUtil.sendAttachmentMail("[杏仁]现金贷支付文件-"+nwDate+"-"+batchNo, "[杏仁]现金贷支付文件-"+nwDate + ",共有交易笔数：" +cnt, file, to, cc);
	}
	/**
	 * 
	 * @return
	 ...
	 * @throws Exception
	 */
	public void readResFile(File resFile) throws Exception {
