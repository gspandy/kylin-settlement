					...
					<div class="main_con">
						<div class="con_article_title02 paddtb20">
							...
							<div class="con_core_info_div05">
								...
								<label class="label_txt wid190 margt11">
									<c:choose>
										<c:when test="${settleTransDetail.dflag==0}">未处理</c:when>
										<c:when test="${settleTransDetail.dflag==1}">处理中</c:when>
										<c:when test="${settleTransDetail.dflag==4}">代收付失败</c:when>
										<c:when test="${settleTransDetail.dflag==40}">代收付失败，结果推送订单失败</c:when>
										<c:when test="${settleTransDetail.dflag==41}">代收付失败，结果推送订单成功</c:when>
										<c:when test="${settleTransDetail.dflag==6}">代收付成功</c:when>
										<c:when test="${settleTransDetail.dflag==60}">代收付成功，结果推送订单失败</c:when>
										<c:when test="${settleTransDetail.dflag==61}">代收付成功，结果推送订单成功</c:when>
										<c:when test="${settleTransDetail.dflag==99}">异常</c:when>
