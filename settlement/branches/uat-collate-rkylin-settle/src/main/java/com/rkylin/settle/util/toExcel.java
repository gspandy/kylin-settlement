...
public class toExcel {
    ...
    throws Exception {
    	...
    	String modelFilename = (String)configMap.get("MODEL_FILE");
    	String filename = (String)configMap.get("FILE");
    	String sheetName = (String)configMap.get("SHEET");
    	String firstDetailRow = (String)configMap.get("firstDetailRow");
    	String multiSheet = (String)configMap.get("multiSheet");
    	String multiSheet_index = (String)configMap.get("multiSheet_index");
    	String firstStyleRow = (String)configMap.get("firstStyleRow");
    			...
    			try{
	    			for(int nRowHead = 0; nRowHead<reportHead.size(); nRowHead++) {
	    				...
	    				if(nRow != -1 && nCol != -1) {
		    				oldRow = sheet.getRow(nRow);
		    				oldCell = oldRow.getCell(nCol);
//							oldCell.setEncoding(HSSFCell.ENCODING_UTF_16); 
...
//				        newCell.setCellStyle((HSSFCellStyle)styleMap.get(String.valueOf(nCol)));
				        ...
				        oldCell.setCellStyle((HSSFCellStyle)styleMap.get(fBuf.toString()));
    			...
    			try{
	    			for(int nRowTail = 0; nRowTail<reportTail.size(); nRowTail++) {
	    				...
	    				if(nRow != -1 && nCol != -1) {
		    				...
		    				oldCell.setCellStyle((HSSFCellStyle)map.get("CELL-STYLE"));
//		    				oldCell.setEncoding(HSSFCell.ENCODING_UTF_16); 
							sVal = (String)map.get("VALUE");
