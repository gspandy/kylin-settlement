...
//		}
	 ...
	 * 现在支付
	 */
	public void collateInowPay() {
		logger.info(">>> >>> >>> 开始: 定时对账, 现在支付代付");
		String payChannelId = SettleConstants.PAY_CHANNEL_ID_INOWPAY;//渠道号S05
		String accountType = SettleConstants.ACCOUNT_TYPE_GENERATE;//对账类型02
		try {
			String merchantCode = Constants.XR_ID;//机构号M000029
			//下载文件并入库
			//collateService.InowPayFileDownAndReadCollateFile( merchantCode, accountType, "2017-06-15", null, payChannelId);
			//账期不传值，默认为当前账期-1
			collateService.InowPayFileDownAndReadCollateFile( merchantCode, accountType, null, null, payChannelId);
			//开始对账
			collateService.collage(payChannelId, accountType, merchantCode, null);
		} catch (Exception e) {
			logger.error(">>> >>> >>> 异常: 定时对账, 现在支付代付");
			logger.error("",e);
			e.printStackTrace();
		}
		logger.info("<<< <<< <<< 结束: 定时对账,现在支付代付");
	}
}
