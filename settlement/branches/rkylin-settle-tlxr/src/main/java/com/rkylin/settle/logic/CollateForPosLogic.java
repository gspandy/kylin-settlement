...
//	private static final String USER_ID_HTGL = "14623543519624388";			//会堂国旅
	private static final String MAIN_ACCOUNT_PRODUCT_ID = "P000264";		//融数钱包_沣盈_保理收款户主账户（沣盈）
	private static final String ORDER_TYPE_REPAY = "W1003";					//订单系统还款单
	private static final String ORDER_TYPE_SETTLE = "W1001";				//订单系统结算单
	private static final String MAIN_PRODUCT_ID = "P000282";				//商单云主账户管理分组
//	private static final String SA_PRODUCT_ID = "P000283";					//商单云待清算账户管理分组
	private static final String[] CONFIRMED_MERCHANT_CODE = new String[]{Constants.RSQB_ID, Constants.FN_ID};
...
//		SimpleDateFormat ymdformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	public Map<String, Object> getTransDetailFromOrder(Date accountDate) throws Exception {
		...
		String[] typesArr = {"W1002", "W1004"};
		whereMap.put("typesArr",typesArr);
		String[] rootsArr = {Constants.FN_ID, Constants.RSQB_ID, Constants.RDPT_ID};
...
//				String goodsname = orderDetail.getGoodsName();
				...
				} else if("W1004".equals(ordertypeid)) {
					settleDetail.setDeliverStatusId(3);//处理状态初始化  不参加还款结算
					settleDetail.setFuncCode("4015");
				} else {
...
//				}
	...
	public Map<String, Object> getPosToSettle(Date invoiceDate) throws Exception {
		...
		try {
			//获取贷款还款账户信息
			accountInfoMap = this.getAccountInfoMap();
		} catch (Exception e) {
		...
		try {
			while(iter.hasNext()) {
				...
				if (keyMap.containsKey(keyStr1)) {
					...
					settleamt = addamt.subtract(addamt1); // 内扣手续费
					tmpBean.setAmount(settleamt.longValue());
					tmpBean.setProductId(productIdMap.get(keyStr));
					tmpBean.setRootInstCd(item.get("MERCHANT_CODE").toString());
					tmpBean.setUserId(userIdMap.get(keyStr));
...
//					}
				...
				if (sumAmtMap.containsKey(keyStr)) {
					...
					sumAmtSubMap.put("USER_ID", userIdMap.get(keyStr));
					sumAmtSubMap.put("PRODUCT_ID", productIdMap.get(keyStr));
...
//						}
		...
		mailContext.append("<BR>");
		logger.info("2.编辑完共有"+ sumAmtMap.size() +"人，需要分润判断！");
		/*
...
//			}
			 ...
			 * 显示结算金额
			 */
			mailContext.append("&nbsp;&nbsp;2-1.入账结算金额为[" + settleAmt + "]");
			mailContext.append("<BR>");
			logger.info("2-1.入账结算金额为[" + settleAmt + "]");
			/*
			 * 修改收单数据状态
			 */
			mailContext.append("&nbsp;&nbsp;2-2.修改收单数据状态");
			mailContext.append("<BR>");
			logger.info("2-2.修改收单数据状态");
			String proId = null;
			...
			if(balance == null) {
				...
				mailContext.append("<BR>");
				logger.info("调用账户接口获取余额成功!"+ argMsg +"，账户余额为：["+amountBalance+"]");
			}
			/*
			 * 结算金额替换为
			 */
			    ...
			    if (shAmt > amt) {
					...
					try {
						...
						if(balance == null) {
							...
							mailContext.append("<BR>");
							logger.info("调用账户接口获取余额成功!返回结果为null "+ argMsg +"，账户余额为：["+amountBalance+"]");
						}
					} catch (Exception e) {
						e.printStackTrace();
						logger.error("调用账户接口获取余额失败!查询余额方法异常  " + argMsg);
					...
					if (shAmt > amt + amountBalance) {
						...
						mailContext.append("<BR>--------------------------------------------------------");
						mailContext.append("<BR>");
						logger.info("金额不满足条件：["+sumAmtSubMap.get("USER_ID").toString() + "]的应还款金额["+shAmt + "],还款金额["+(amt+amountBalance)+"],差额["+(shAmt-(amt+amountBalance))+"]");
						logger.info("--------------------------------------------------------");
						continue;
...
//						}
						...
						mailContext.append("<BR>");
						logger.info("金额满足条件：["+sumAmtSubMap.get("USER_ID").toString() + "],但还应从账户支出：["+accAmt+"]");
					}
			    }
			    mailContext.append("&nbsp;&nbsp;金额满足条件：["+sumAmtSubMap.get("USER_ID").toString() + "]的应还款金额["+shAmt + "],还款金额["+amt+"]");
			    mailContext.append("<BR>--------------------------------------------------------");
			    mailContext.append("<BR>");
			    logger.info("金额满足条件：["+sumAmtSubMap.get("USER_ID").toString() + "]的应还款金额["+shAmt + "],还款金额["+amt+"]");
		...
		try {
			for (int i = 0; i < amtList.size(); i++) {
				...
				try {
					...
					mailContext.append("<BR>");
					logger.info("计息异常: ["+sumAmtSubMap.get("MERCHANT_CODE").toString()+"|" + sumAmtSubMap.get("USER_ID").toString() + "]调用计息和账户都失败"+"计息参数：["+ymdformat.format(accountDate)+"|"+amt+"]账户参数：["+ sumAmtSubMap.get("INTO_USER_ID") +"|"+ productId +"|"+sumAmtSubMap.get("AMOUNT").toString()+"|1]");
					e.printStackTrace();
				}
				if ((subRtnMap == null || "false".equals(subRtnMap.get("code").toString()))) {
				...
				if (accAmt > 0) {
					...
					mailContext.append("<BR>");
					logger.info("账户转账: ["+accAmt+"] > 0所以进入转账流程");
					try {
						...
						if((subARtnMap == null || "0".equals(subARtnMap.get("code").toString()))) {
							...
							logger.info("账户转账: ["+sumAmtSubMap.get("MERCHANT_CODE").toString()+"|" + sumAmtSubMap.get("USER_ID").toString()+"|"+  sumAmtSubMap.get("INTO_USER_ID").toString()  +"|" + accAmt+"|"+ productId +"|"+ intoProductId +"],["+ subARtnMap.get("code") +"|"+ subARtnMap.get("msg") +"]调用失败");
						} else {
							...
							logger.info("账户转账: ["+sumAmtSubMap.get("MERCHANT_CODE").toString()+"|" + sumAmtSubMap.get("USER_ID").toString()+"|"+  sumAmtSubMap.get("INTO_USER_ID").toString()  +"|" + accAmt+"|"+ productId +"|"+ intoProductId +"],["+ subARtnMap.get("code") +"|"+ subARtnMap.get("msg") +"]调用完成!");
						}
					} catch (Exception e) {
						...
						e.printStackTrace();
						logger.info("账户异常: ["+sumAmtSubMap.get("MERCHANT_CODE").toString()+"|" + sumAmtSubMap.get("USER_ID").toString()+"|"+ sumAmtSubMap.get("INTO_USER_ID").toString() +"|" + accAmt+"|"+ productId +"|"+ intoProductId +"]调用异常");
					}
				} else {
...
//		RkylinMailUtil.sendMail("POS收单结束-"+mailDate, mailContext.toString(), mailTo, mailCc);
		...
		logger.info(">>> >>> >>> >>> 根据收单交易调用计息&账户接口 END <<< <<< <<< <<<");
		return resultMap;
	}
	/***
	 * 更新 分润结果表状态 为'已结算'
...
//	}
	...
	private Map<String, Map<String, String>> getAccountInfoMap() throws Exception {
		...
		for(SettleParameterInfo parameterInfo : list) {
			...
			sAProductIdMap.put(merchantCode + sAUserId, sAProductId);
			sAUserIdMap.put(merchantCode + sAUserId, sAUserId);
			factorUserIdMap.put(merchantCode + sAUserId, factorUserId);
			factorProductIdMap.put(merchantCode + sAUserId, factorProductId);
			factorTitleMap.put(merchantCode + sAUserId, factorTitle);
			merchantMap.put(merchantCode + sAUserId, merchantCode);
			userIdMap.put(merchantCode + sAUserId, userId);
			productIdMap.put(merchantCode + sAUserId, productId);
		}		
...
//	}
	...
	private void queryAccountInfoByPosDetail(List<Map<String,Object>> settlePosDetailList) throws Exception {
		...
		String productId = MAIN_PRODUCT_ID;		//供应商主账户管理分组
		String sAUserId = null;					//供应商待清算UserId
		String sAProductId = null;		//供应商待清算管理分组
		...
		for(Map<String,Object> settlePosDetailMap : settlePosDetailList) {
			...
			sAProductId = String.valueOf(settlePosDetailMap.get("PRODUCT_ID"));
