 ...
 * @author Yang
 */
@Component("collateforposLogic2")
public class CollateForPosLogic2 extends BasicLogic2 {
	@Autowired
...
//	private String[] mailCc = new String[]{"youyu@rongcapital.cn","sunruibin@rongcapital.cn"};
...
//				return this.editResultMap(resultMap, "0", "获取 清算交易信息-对账结构 0条记录");
		...
		for(String ruleKey : detailMapStru.keySet()) {//遍历'清算'方对账结构体
			...
			settleBalanceEntry.setBalanceType(dataFrom);
	...
	private void updateAccAndDetTransStatus(List<Map<String, Object>> colStatInfoList) {
		...
		if(succDetIdList.size() > 0) {
			...
			settleFinanaceAccountManager.updateTransStatusId(map);
		}
...
//					throw new SettleException("'上游or清算的对账信息'的Map中,key'"+ key +"'所对应的value为'null'");
	...
	public Map<String, Object> transferInCommon(long amount,String userId,String interUserId,String rootInstCd,String productId,String intoProductId) throws Exception {
		...
		try {
			 ...
			 */
			logger.info(">>> 执行账户操作!");
			TransOrderInfo transOrderInfo = this.editTransOrderInfoByparaMap(paraMap);
...
//		SimpleDateFormat ymdformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	public Map<String, Object> getTransDetailFromOrder(Date accountDate) throws Exception {
		...
		String[] rootsArr = {Constants.RSQB_ID};
		whereMap.put("rootsArr",rootsArr);
		whereMap.put("	",accountDate);
		//查询'订单'交易信息
		try {
...
//				}
	...
	public Map<String, Object> getPosToSettle(Date invoiceDate) throws Exception {
		...
		try {
			//获取未处理的还款交易
			settlePosDetailList = this.getPosDetailForProfitByMap(accountDateForP, rootInstCdArr);
			...
			this.editResultMap(resultMap, "-1", msg);
			e.printStackTrace();
			RkylinMailUtil.sendMail("POS收单异常-"+mailDate, e.getMessage(), mailTo, mailCc);
			return resultMap;
		}
		try {
		...
		try {
			//获取贷款还款账户信息
			accountInfoMap = getAccountInfoMap(rootInstCdArr);
		} catch (Exception e) {
		...
		try {
			...
			if (sumAmtMap.keySet().size() < userIdMap.keySet().size()) {
			...
			if(settleAmt > 0L) {
				try {
					message = "参数：[" + sumAmtSubMap.get("INTO_USER_ID") 
								...
								+ "|" + settleAmt+"|1]";
					changeAccountResultMap = this.changeAccount2P(
			...
			if (transIdMapByUpdate.containsKey(key)) {
				...
				if(idList.size() > 0) {
					try {
						...
						RkylinMailUtil.sendMail("POS收单异常-"+mailDate, mailContext.toString(), mailTo, mailCc);
						return resultMap;
					}
...
//		RkylinMailUtil.sendMail("POS收单结束-"+mailDate, mailContext.toString(), mailTo, mailCc);
		...
		RkylinMailUtil.sendAttachmentMail("POS收单结束-"+mailDate, "POS收单详情见附件: " + ATTACHMENT_NAME, attachment, mailTo, mailCc);
		return resultMap;
	}
	/***
	 * 更新 分润结果表状态 为'已结算'
...
//	}
	 ...
	 * @return
	 */
	private Map<String, Map<String, String>> getAccountInfoMap(String[] rootInstCdArr) throws Exception {
		...
		for(SettleParameterInfo parameterInfo : list) {
			//兼容二期 排除非二期所有机构
			if(!Arrays.asList(rootInstCdArr).contains(parameterInfo.getProductId())) continue;
	 ...
	 * @return 
	 */
	private BalanceInfo getBalance(String userId, String productId, String constId, String paramString) throws Exception {
...
//		transOrderInfo.setUserIpAddress(SettleConstants.LOCALHOST_URL);
	...
	private void queryAccountInfoByPosDetail(List<Map<String,Object>> settlePosDetailList) throws Exception {
		...
		for(Map<String,Object> settlePosDetailMap : settlePosDetailList) {rootInstCd = String.valueOf(settlePosDetailMap.get("MERCHANT_CODE"));
			/*
		...
		if(settleParameterInfoList == null || settleParameterInfoList.size() < 1) {
			...
			for(String key : accountInfoMap.keySet()) {
				...
				settleParameterInfo.setProductId(itemMap.get("userId"));
				settleParameterInfo.setParameterCode(itemMap.get("sAUserId"));
				...
				settleParameterInfo.setRemark(itemMap.get("factorTitle") + "&" + itemMap.get("factorProductId"));
			...
			for(String key : accountInfoMap.keySet()) {
				...
				for(SettleParameterInfo itemParam : settleParameterInfoList) {
					...
					if(key.equals(keyStr)) {
						...
						itemParam.setProductId(itemMap.get("userId"));
						itemParam.setParameterCode(itemMap.get("sAUserId"));
				...
				settleParameterInfo.setObligate2(itemMap.get("sAProductId"));
				settleParameterInfo.setObligate3(itemMap.get("factorUserId"));
				settleParameterInfo.setRemark(itemMap.get("factorTitle"));
				settleParameterInfoManager.saveSettleParameterInfo(settleParameterInfo);
			}
		}
	 ...
	 * @return
	 */
	private TransOrderInfo editTransOrderInfoByparaMap(Map<String,String[]> paraMap) {
		return null;
	}
}
