		...
		*/
		...
		if(settleTransInvoiceList != null && settleTransInvoiceList.size() > 0) {
			for(int i = 0; i < settleTransInvoiceList.size(); i ++) {
				...
				province = sti.getProvince()==null?"":sti.getProvince();
...
//				city = sti.getCity()==null?"":sti.getCity();
...
//		String[] cc = new String[]{"caoyang@rkylin.com.cn","sunruibin@rongcapital.cn","liqiuying@rongcapital.cn"};
		...
		for(SettleTransInvoice bean : settleTransInvoiceList){
			...
			bean.setRemark("已发送 文件:" + fileName);
...
//		txtReader.setEncode("UTF-8");
	 ...
	 * @throws Exception 
	 */
	public void batchInsertTransSummery(List<SettleTransSummary> transSummeryList) throws Exception {
		logger.info(">>> >>> >>> 开始:批量向汇总表中插入数据");
		final Integer total = transSummeryList.size();//总数量
		if(total < 1) {
			logger.info(">>> >>> 批量插入汇总表条数为0条!");
			return;
		}
		...
		while(totalIndex >= batchIndex) {
			...
			settleTransSummaryManager.batchInsertTransSummery(transSummeryList.subList(fromIndex, toIndex));
			batchIndex ++;
		}
		logger.info("<<< <<< <<< 结束:批量向汇总表中插入数据");
	}
	/**
	 * 批量插入或更新结算表中的数据
	 * @throws Exception 
	 */
	public void batchInsertTransInvoice(List<SettleTransInvoice> transInvoiceList) throws Exception {
		logger.info(">>> >>> >>> 开始:批量插入或更新结算表中的数据");
		final Integer total = transInvoiceList.size();//总数量
		if(total < 1) {
			logger.info(">>> >>> 批量插入结算表条数为0条!");
			return;
		}
		final Integer size = 500;//每批数量
		final Integer totalIndex = total / size + 1;//总批数
		...
		while(totalIndex >= batchIndex) {
			fromIndex = (batchIndex - 1) * size;
			toIndex = totalIndex == batchIndex ? total : batchIndex * size;
			settleTransInvoiceManager.batchInsertTransInvoice(transInvoiceList.subList(fromIndex, toIndex));
			batchIndex ++;
		}
		logger.info("<<< <<< <<< 结束:批量插入或更新结算表中的数据");
	}
}