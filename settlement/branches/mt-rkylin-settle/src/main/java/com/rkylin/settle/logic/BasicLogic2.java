 ...
 * @author Yang
 */
@Component("basicLogic2")
public class BasicLogic2 {
	//日志对象
	protected static Logger logger = LoggerFactory.getLogger(BasicLogic2.class);
	//调单边账DEAL_PRODUCT_ID
//	private static final String CHECK_THE_BALANCE_DPI = "PROD_00_RS_0066";
	...
	@Autowired
	SettlePosDetailManager settlePosDetailManager;
//	@Autowired
...
//					throw new SettleException("'上游or清算的对账信息'的Map中,key'"+ key +"'所对应的value为'null'");
	 ...
	 * @throws Exception
	 */
	protected Map<String, Object> changeAccount1(String instCode,String userId,String productId,long amount,int status) throws Exception {
...
//		rtnMap.put("msg", commonResponse.msg);
	 ...
	 * @return
	 */
	protected List<SettleTransProfit> getTransProfitWithUnbalance(Date settleTime) {
		return getTransProfitWithUnbalance(settleTime, null);
	}
	/***
	 * 查询全部未结算的清分交易
	 * String[] rootInstCdArr机构号
	 * @return
	 */
	protected List<SettleTransProfit> getTransProfitWithUnbalance(Date settleTime, String[] rootInstCdArr) {
		...
		queryMap.put("rootInstCdArr", rootInstCdArr);
		return settleTransProfitManager.selectloanProfitWithUnbalance(queryMap);
	}
	/***
	 * 查询机构集合
	 * @param String accountSysCd 系统编号
	 */
	protected String[] queryRootInstCdArr(String accountSysCd) throws Exception {
		String[] rootInstCdArr = null;
		SettleParameterInfoQuery query = new SettleParameterInfoQuery();
		query.setParameterType(SettleConstants.PARAMETER_DOWN_MERCHANT);
		query.setObligate2(accountSysCd);
		query.setStatusId(1);
		List<SettleParameterInfo> list = settleParameterInfoManager.queryList(query);
		if(list == null || list.size() < 1) throw new Exception("查询机构集合 is null or size < 1");
		rootInstCdArr = new String[list.size()];
		for(int i = 0; i < rootInstCdArr.length; i ++) {
			rootInstCdArr[i] = list.get(i).getParameterCode();
		}
		return rootInstCdArr;
	}
	/***
	 * 查询全部处理过的清分交易
	 * @return
	 */
	public List<Map<String,Object>> getPosDetailForProfitByMap(Date settleTime, String[] rootInstCdArr) {
		logger.info(">>> >>> >>> >>> 查询全部未处理的收单交易!");
		Map<String, Object> query = new HashMap<String, Object>();
		query.put("deliverStatusId", 1);
		query.put("accountDate", settleTime);
		query.put("statusId", 21);
		query.put("rootInstCdArr", rootInstCdArr);
		return settlePosDetailManager.queryPosForProfitListByMap(query);
	}
}
