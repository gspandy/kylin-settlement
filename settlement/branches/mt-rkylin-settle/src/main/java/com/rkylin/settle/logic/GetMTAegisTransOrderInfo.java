...
public class GetMTAegisTransOrderInfo extends BasicLogic {
	...
	private SettleTransDetail transOrderInfo2SettleTransDetail(
		...
		String dealProductCodesPara = null;
		String dealProductCodeTrans = null;
		//遍历对应关系
		...
		while(parameterIter.hasNext()) {
			 ...
			 * 判断匹配方式
			 */
			if(funcCode.indexOf("_*") != -1) {//统配符号匹配
				dealProductCodesPara = dealProductCodesPara.replace("*", "");
				dealProductCodeTrans = dealProductCodeTrans.substring(0, dealProductCodeTrans.lastIndexOf("_") + 1);
			} else {//精确匹配
				dealProductCodesPara = parameterInfo.getParameterValue();//deal_product_code
				dealProductCodeTrans = settleTransDetail.getDealProductCode();
			}
			/*
			 * 获取funcCode
			 */
			if((dealProductCodesPara).indexOf(dealProductCodeTrans) != -1){
...
//    		}
	...
	public void updateDflag(SettleTransDetail settleTransDetail){
		...
		if (",4013,4014,4014_1,4016,40143,40144,40147,40149,".indexOf("," + settleTransDetail.getFuncCode()+ ",") != -1){
