...
public class DsfMessageListener extends BaseDao implements MessageListener {
	...
	public void onMessage(Message msg) {
		...
		if (SettleConstants.DSF_HAS_WRITE != invoiceBean.getStatusId()) {//监听MQ队列,statusId!=6 没有回写过数据,进行发送
			/*
			 * 将代收付结果更新到汇总表
			 */
			try {
	...
	public void returnAccount(Map<String, List<SettleTransDetail>> detailList){
		try{
			...
			List<TransOrderInfoResult> param2DfList = new ArrayList<TransOrderInfoResult>();//代付2期
			List<TransOrderInfoResult> param2DsList = new ArrayList<TransOrderInfoResult>();//代收2期
...
//			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			...
			for (SettleTransSummary summaryBean : summaryList) {
				...
				if (((hour == 18 && minute >= 30) || hour > 18) 				//18:30之后交易结果立即发给账户/订单
						|| (dflag == 6 && detailBean != null)					//或 成功交易的立即发给账户/订单
						|| (Constants.XR_ID.equals(invoiceBean.getRootInstCd()) //或 现金贷的代付(M000029,40149)立即发给账户/订单
								&& "40149".equals(invoiceBean.getFuncCode())
					)) {
...
//		List<String> queryProfitFuncCodes = settleTransDetailService.queryProfitFuncCodes();
		...
		TransOrderInfoBack transOrderInfo = null;// mtaegis交易结构体
		String code = null;
		String msg = null;
...
//				transOrderInfo.setUserId(userId);
	 ...
	 */
	public void splitAccountData(Map<String, List<SettleTransDetail>> detailList,List<com.rkylin.wheatfield.bean.TransOrderInfo> param1List,List<TransOrderInfoResult> param2DfList,List<TransOrderInfoResult> param2DsList){
		try {
			if(detailList == null || detailList.size()==0){
			...
			TransOrderInfoResult transOrderInfo = new TransOrderInfoResult();
			...
			} else if (detailList.containsKey("2")){ // 账户2期数据
				subList = detailList.get("2");
				for(SettleTransDetail detailBean : subList){
					transOrderInfo = new TransOrderInfoResult();
					...
					if(SettleConstants.BATCH_DF_FUN_CODE.equals(detailBean.getFuncCode()) 
							...
							|| SettleConstants.BATCH_DF3_FUN_CODE.equals(detailBean.getFuncCode())
							|| SettleConstants.BATCH_DF4_FUN_CODE.equals(detailBean.getFuncCode())) {
			...
			logger.error("拆分交易表的数据，决定具体调用账户的哪个接口进行代收付结果的推送发生异常,error="+e, e);
		}
	}
	/**
	 * 查询账户DealProductCode 和 funcCode的关系
	 * @return
	 */
	private Map<String, String> getDealProductCodeTofuncCodeMap() {
		Map<String, String> resultMap = new HashMap<String, String>();
		List<SettleParameterInfo> paramList = null;
		paramList = logicConstantUtil.getDealProductCodeTofuncCode();
		if(paramList == null || paramList.size() < 1) {
			logger.error(">>> >>> getDealProductCodeTofuncCodeMap paramList is Null || paramList.size < 1");
			return resultMap;
		}
		
		for(SettleParameterInfo item : paramList) {
			resultMap.put(item.getParameterCode(), item.getParameterValue());
		}
		return resultMap;
	}
	/**
	...
	public void dealDsResultTo2Account(List<TransOrderInfoResult> param2DsList){
		try{
			...
			logger.error("组织账户二期代收结果即将发往账户的过程中发生异常"+e, e);
		}
	}
	 ...
	 */
	public void dealDfResultTo2Account(List<TransOrderInfoResult> param2DfList){
		...
		try{
			...
			logger.error("组织账户二期代收付结果即将发往账户的过程中发生异常"+e, e);
		}
	}
	...
	public void updateSend1AccountFail(
		...
		try {
			...
			logger.error("更新账户一期代收付结果推送给账户失败的数据发生异常=" + e, e);
		}
	...
	public void updateSend1AccountSuccess(
		...
		try {
			...
			logger.error("更新账户一期代收付结果推送给账户成功的数据发生异常，error=" + e, e);
		}
	}
	/**
	...
	public void updateDflagByAccountSuccess(List<TransOrderInfoResult> paramList){
		try{
			...
			logger.error("更新账户二期代收付结果推送给账户成功的数据发生异常，error="+e, e);
		}
	}
	/**
	...
	public void updateDflagByAccountFail(List<TransOrderInfoResult> paramList,List<MultTransOrderInfoReturn> rtnList){
		try{
			...
			logger.error("更新账户二期代收付结果推送给账户失败的数据发生异常，error="+e, e);
		}
	}
	/**
	 * 返回代付结果给MQ
