...
public class ToExcel2007S {
    ...
    throws Exception {
    	...
    	String modelFilename = String.valueOf(configMap.get("MODEL_FILE"));			//模板 绝对路径
    	String filename = String.valueOf(configMap.get("FILE"));					//目标文件 绝对路径
    	String sheetName = String.valueOf(configMap.get("SHEET"));					//sheet页名字 唯一
    	String firstDetailRow = String.valueOf(configMap.get("firstDetailRow"));	//详细row 开始索引
    	String multiSheet = String.valueOf(configMap.get("multiSheet"));			
    	String multiSheet_index = String.valueOf(configMap.get("multiSheet_index"));
    	String firstStyleRow = String.valueOf(configMap.get("firstStyleRow"));		//明细row样式 开始索引
    		...
    		if( reportTail != null && reportTail.size() > 0){
    			try{
	    			for(int nRowTail = 0; nRowTail<reportTail.size(); nRowTail++) {
	    				...
	    				nCol = -1;
	    				try{
	    					nRow = Integer.parseInt(String.valueOf(map.get("ROW")));
	    					nCol = Integer.parseInt(String.valueOf(map.get("COL")));
	    				}catch(Exception ex){
		    				nRow = -1;
		    				nCol = -1;	    					
	    				}
...
//							oldCell.setEncoding(XSSFCell.ENCODING_UTF_16); 
							sVal = String.valueOf(map.get("VALUE"));
    		...
    		try{
				for(nRow = 0; nRow<detailList.size(); nRow++) {			
		            ...
		            if(map.containsKey("FONT-SET")){
		            	try{
		            		nStyleIndex = Integer.parseInt(String.valueOf(map.get("FONT-SET")));
		            	}catch(Exception ex){
		            		;
		            	}
		            	map.remove("FONT-SET");
...
//				        newCell.setCellStyle((XSSFCellStyle)styleMap.get(String.valueOf(nCol)));
						...
						sVal = String.valueOf(map.get(fBuf.toString()));
    			...
    			try{
	    			for(int nRowTail = 0; nRowTail<reportTail.size(); nRowTail++) {
	    				...
	    				if(nRow != -1 && nCol != -1) {
		    				...
		    				oldCell.setCellType(Short.parseShort(String.valueOf(map.get("CELL-TYPE"))));
    		...
    		if(reportMerge != null && reportMerge.size() > 0){
    			try{
    				...
    				for(int nRowTail = 0; nRowTail<reportMerge.size(); nRowTail++) {
	    				...
	    				nColFrom = -1;
	    				nColTo = -1;		    				
	    				nRowFrom = Integer.parseInt(String.valueOf(map.get("ROW_FROM")));
	    				nRowTo = Integer.parseInt(String.valueOf(map.get("ROW_TO")));
	    				nColFrom = Integer.parseInt(String.valueOf(map.get("COL_FROM")));
	    				nColTo = Integer.parseInt(String.valueOf(map.get("COL_TO")));
