...
@Scope("prototype")
public class SettleTransDetailController extends BaseController {
	...
	public void queryDsfAjax(SettleTransDetailQuery query) throws Exception {
		try {
			...
			if(StringUtils.isBlank(query.getFuncCode())){
				query.setFuncCodes(new String[]{"4013","4014","4014_1","4016","40143","40144","40147","40149"});
			}
   ...
   /***
	 ...
	 * @param ids
	 */
	@RequestMapping("/collecte_acc_and_multi")//TODO
	public void collecteAccAndMulti(String inRootInstCd, Date accountDate,String readType) throws Exception {
		String resultMsg = "";
		...
		try {
			System.out.println("inRootInstCd:"+inRootInstCd);
			System.out.println("accountDate:"+accountDate);
			System.out.println("readType:"+readType);
			settleWebInterface.collecteAccAndMulti(inRootInstCd, accountDate,readType);
			resultMsg = "success";
		} catch (Exception e) {
			e.printStackTrace();
   ...
   /***
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/update_status_by_record")
	public String updateStatusByRecord(){
		return "/settlement/trans/detail/jx_update_status";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/jx_update_status")
	public String jxUpdateStatus(String totalAmount,Date payMentDate,String userId,String merchantCode){
		String result = "success";
		try {
			if(StringUtils.isBlank(totalAmount)) totalAmount = "0";
			if(StringUtils.isBlank(userId)) userId = null;
			if(StringUtils.isBlank(merchantCode)) merchantCode = null;
			Long totalAmountP = Long.parseLong(totalAmount); 
			Map<String, Object> map = settleWebInterface.updateStatusByRecord(merchantCode, userId, payMentDate, totalAmountP);
		    Boolean isSuccess =  (Boolean) map.get("code");
		    if(!isSuccess) result = "fail";
		} catch (Exception e) {
			logger.error("调用计息更新状态接口错误信息："+ e);
			result = "fail";
		}
		return result;
	}
