 ...
 * 验证入参并结算
 */
function valieQueryTimeDoSettle() {
	var $accountDate = $("#accountDate");
	if($accountDate && $accountDate.val()) {
		doSettle();
	} else {
 ...
 */
function doSettle() {
	loadingFun("settleSubmit");
	$.post(
		$('#settle_ajax').val(),										//发送请求 url
		"accountDateStr="+$('#accountDate').val(),										//表单数据
		function(data) {
			successedFun("settleSubmit", "结算");
			art.dialog.tip1s(data,5);
		},
		'text'
	);
}
 ...
 * 
 */
function loadingFun(buttId) {
	buttId = buttId ? buttId : "btnSubmit";
	var butt = document.getElementById(buttId);
	butt.onclick = function() {};
	butt.style.width = "100px";
	butt.innerHTML = "正在处理 ... ...";
}
function successedFun(buttId, txt) {
	buttId = buttId ? buttId : "btnSubmit";
	txt = txt ? txt : "开始对账";
	var butt = document.getElementById(buttId);
	butt.style.width = "100px";
