...
public interface SettleProfitRuleDao {
	...
	List<SettleProfitRule> selectByPreExample(SettleProfitRuleQuery example);
	
	int updateByUnionKeySelective(SettleProfitRule record);
	
	List<SettleProfitRule> findByprofitDetailId(String profitDetailId);
	
	int updateProfitDetailIdByMatch(SettleProfitRuleQuery example);
}
